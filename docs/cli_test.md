...
```
1. Checking invalid interface vlan input parameters (vlanid: 2abc & abc2abc & abc#$)
Run commands:
```
...
```
1. Deleting interface vlan outside range <1-4094>
Run commands:
```
...
#### Test Result Criteria
##### Test Pass Criteria
1. CLI should throw an error specifying that the VLAN is out of range
2. Successfully verify invalid vlanid (vlanid: 2abc & abc2abc & abc#$)
3. CLI should throw an error specifying that the VLAN is out of range
##### Test Fail Criteria
...
Single Switch Topology
#### Description
1. Adding vlan interface and verify if VRF can see it
Run commands:
```
root(config)interface vlan 1
root(config)do show vrf
```
1. Verify we see the interface and port created in OVSDB and attached to bridge with name same as Interface Name and type as internal.
	* Verify interface exists in ovsdb
	```
	...
	* Verify port is in bridge
	...
	```
	* Verify port is in vrf
	```
	/usr/bin/ovs-vsctl get port vlan%s _uuid
	/usr/bin/ovs-vsctl get vrf vrf_default ports
	```
1. Deleting interface vlan and verify if VRF can see it
Run commands:
```
root(config)no interface vlan 1
root(config)do show vrf
```
1. Deleting vlan interface from OVSDB with name same as Interface Name
	...
	* Check for port name in OVSDB
	...
	/usr/bin/ovs-vsctl get port vlan%s name
	```
1. Checking multiple interfaces add and delete
#### Test Result Criteria
##### Test Pass Criteria
1. Add vlan interface and attach to VRF
1. On successful configuration, OVSDB should have a port row and a corresponding interface of type internal
1. Deleting vlan interface and detach from VRF
1. On vlan interface delete, the port and interface should not be present in OVSDB
1. Successfully adds and deletes these interfaces
##### Test Fail Criteria

### Modify Vlan interface
#### Objective
Modifying vlan interface configuration to test show running-config
#### Requirements
- Virtual Mininet Test Setup
- **CT file**: ops-cli/vtysh/tests/test_vtysh_ct_intervlan.py
...
#### Test Result Criteria
##### Test Pass Criteria
IP address should reflect for in show running-config
##### Test Fail Criteria
