...
int check_cmd_authorization(char *tac_command)
{
  ...
  /* check if the number of groups configured is more than one(default) and first groups is not "none" */
  if (count > 1 && (strcmp(group_prio_list->value_authorization_group_prios[0]->group_name, TAC_NONE_GROUP) != 0))
  {
    for (iter = 0; iter < count; iter ++)
    {
      ...
      if (strcmp(group_row->group_name, TAC_NONE_GROUP) != 0)
      {
        ...
        /* sort the list by default priority if the groups is default */
        if (strcmp(group_row->group_name, TAC_DEFAULT_GROUP) == 0)
        {
          nodes = sort_tacacs_server(&sorted_tacacs_servers, true);
        }
        else
        {
          /* else get the list sorted by group priority*/
          nodes = sort_tacacs_server(&sorted_tacacs_servers, false);
        }
