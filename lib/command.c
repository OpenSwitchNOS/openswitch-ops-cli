...
int check_cmd_authorization(char *tac_command, const struct
                            ovsrec_aaa_server_group_prio *group_prio_list,
                            struct vty *vty)
{
  ...
  if (group_prio_list != NULL)
  {
    ...
    if (count >= 1 && strcmp(group_prio_list->value_authorization_group_prios[0]->group_name, TAC_NONE_GROUP))
    {
			  ...
			  struct cmd_element **cmd)
{
  ...
  tac_author_return = check_cmd_authorization(tac_command, group_prio_list, vty);
  if (tac_author_return == EXIT_FAIL)
  {
    vty_out(vty, "Error: You are not authorized to execute this command\n");
