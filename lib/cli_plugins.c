...
/* plugin_class structre used for calling feature specific
 ...
 * Parameters : const char *filename: module filename which is passed by libltdl.
 ...
 * Parameters : const char *path:libltdl serach path.
 * Return : void.
 */
void
plugins_cli_init(const char *path)
{
    ...
    plugins_path=(char*)malloc(len);
    strcpy(plugins_path,path);

    /* Initialize libltdl, set the libltdl serach path,
     * initialize advise parameter,which is used to module loader
     * when using lt_dlopenadvise to perform the loading.
     */
    if (lt_dlinit() ||
