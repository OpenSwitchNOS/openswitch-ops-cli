...
cli_neighbor_aspath_filter_cmd_execute(char *vrf_name, char *ip_addr, char *name, char *dir)
{
    ...
    if (!fl_found) {
        ERRONEOUS_DB_TXN(txn, "ASPath filter doesn't exist");
    }

    int num_entries = ovs_bgp_neighbor->n_aspath_filters;
    char **direction = xmalloc(sizeof(*direction) * (num_entries+1));
    struct ovsrec_bgp_aspath_filter **fl_table_entry = xmalloc(sizeof(*fl_table_entry) *
            (num_entries+1));
        ...
        char *dir)
{
    ...
    if (!ovs_bgp_neighbor->n_aspath_filters) {
        ABORT_DB_TXN(txn, "no existing neighbor filter-list to unset");
    }

    /* Check to see if a filter-list is configured for the direction. */
    int num_entries = ovs_bgp_neighbor->n_aspath_filters;
    char **direction = xmalloc(sizeof(*direction) * num_entries);
    struct ovsrec_bgp_aspath_filter **fl_table_entry = xmalloc(sizeof(*fl_table_entry) * num_entries);
    char *direct;
    ...
    if (!dir_found) {
        free(direction);
        free(fl_table_entry);
        ABORT_DB_TXN(txn, "neighbor filter-list for the direction doesn't exist");
    }
...
static bool get_aspath_filter_match(const struct ovsrec_bgp_aspath_filter *fl_row, const char *des, const char* action)
{
    int itr;
                                ...
                                const char *name, const char *action, const char *description)
{
    ...
    if (!fl_row) {
        OVSREC_BGP_ASPATH_FILTER_FOR_EACH(fl_row, idl) {
            if (strcmp(fl_row->name, name) == 0) {
                if (!strcmp(action, "permit")) {
                    int num_entries = fl_row->n_permit;
        ...
        if(entry_set != true) {
            ...
            if (!strcmp(action, "permit")) {
                int num_entries = 0;
                char **permit_entries = xmalloc(sizeof(*permit_entries) * (num_entries+1));
            ...
            if (!strcmp(action, "deny")) {
                int num_entries = 0;
                char **deny_entries = xmalloc(sizeof(*deny_entries) * (num_entries+1));
