                      ...
                      const struct ovsrec_bgp_neighbor *ovs_bgp_neighbor)
{
    ...
    if (ovs_bgp_neighbor->bfd_fallover_enable) {
        vty_out(vty, "    bfd_fallover_enable: %s\n",
                safe_print_bool(ovs_bgp_neighbor->n_bfd_fallover_enable, ovs_bgp_neighbor->bfd_fallover_enable));

	/* bfd status */
        vty_out(vty, "    BFD status: %s\n",
                safe_print_string(1, ovs_bgp_neighbor->bfd_fallover_status));
    }
