                                         ...
                                         int asn)
{
    ...
    int i = 0;
    if (!ovs_bgpr) {
        VLOG_DBG("Router ctxt is NULL for get neighbor from asn\n");
        return NULL;
    }
      ...
      "Soft reconfig outbound update\n")
{
    ...
    if (vrf_row == NULL) {
        ...
        VLOG_DBG("Vrf row is NULL\n");
        cli_do_config_abort(status_txn);
        return CMD_OVSDB_FAILURE;
    }

    row = get_ovsrec_bgp_router(vrf_row);
    if(!row)
    {
        VLOG_ERR(OVSDB_ROW_FETCH_ERROR);
    ...
    OVSREC_VRF_FOR_EACH(ovs_vrf, idl) {
        for (i = 0; i < ovs_vrf->n_bgp_routers; i++) {
            row = ovs_vrf->value_bgp_routers[i];
      ...
      "Soft reconfig outbound update\n")
{
    ...
    bgp_router_row = get_ovsrec_bgp_router(vrf_row);
    if(!bgp_router_row)
    {
        VLOG_ERR(OVSDB_ROW_FETCH_ERROR);
        VLOG_DBG("BGP router row does not exist\n");
    ...
    smap_replace(&smap_status, "bgp_num_clear_counters_peer_soft_out_requested",
                 buffer);
      ...
      "Soft reconfig inbound update\n")
{
    ...
    if ((clear_bgp_neighbor_table_requested < clear_bgp_neighbor_table_performed)) {
        VLOG_DBG("Counter for requested clear bgp neighbor table %d"
                 "lags counter for performed clear bgp neighbor"
                 "table %d by %d\n", clear_bgp_neighbor_table_requested,
                 clear_bgp_neighbor_table_performed,
                 clear_bgp_neighbor_table_performed - clear_bgp_neighbor_table_requested);
        cli_do_config_abort(status_txn);
        return CMD_OVSDB_FAILURE;
    }
    ...
    snprintf(buffer, BUF_LEN-1, "%d", clear_bgp_neighbor_table_requested);
    smap_clone(&smap_status, &row->status);
    smap_replace(&smap_status, "bgp_num_clear_counters_peer_soft_in_requested",
                 buffer);
    ovsrec_bgp_neighbor_set_status(row, &smap_status);
