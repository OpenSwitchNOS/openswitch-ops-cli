...
static const struct lookup_entry set_table[] = {
    ...
    {"comm-list", "comm_list"},
    {"extcommunity rt", "extcommunity_rt"},
    {"extcommunity soo", "extcommunity_soo"},
                            ...
                            route_psd_bgp_t *data)
{
    ...
    data->weight = smap_get_int(&rib_row->path_attributes,
                                    OVSDB_BGP_ROUTE_PATH_ATTRIBUTES_WEIGHT, 32768);
    ...
    for (kk = 0; kk < count; kk++) {
        ...
        if (rib_row->prefix) {
            ...
            if (!strcmp(rib_row->address_family,
                ...
                vty_out (vty, ", weight %d ", ppsd->weight?ppsd->weight:32768);
                /* Print AS path. */
                if (ppsd->aspath) {
                  ...
                  boolean print_header)
{
    ...
    if (ppsd->flags & BGP_INFO_STALE)
        vty_out (vty, ", (stale)");
    if (ppsd->aggregator_id != 0 &&  strcmp(ppsd->aggregator_addr, "")) {
    ...
    if (ppsd->flags & BGP_INFO_SELECTED)
        vty_out (vty, ", best");
    vty_out (vty, "%s", VTY_NEWLINE);
    if (strcmp(ppsd->community, "")) {
        vty_out (vty, "       Community: %s", ppsd->community);
        vty_out (vty, "%s", VTY_NEWLINE);
    }
    if (strcmp(ppsd->ecommunity, "")) {
