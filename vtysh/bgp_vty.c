...
cli_bgp_show_summary_vty_execute(struct vty *vty, int afi, int safi)
{
    ...
    count = bgp_get_rib_count();
    vty_out(vty, "RIB entries %ld, using %s of memory%s", count,
            mtype_memstr (memstrbuf, sizeof (memstrbuf),
                          count * sizeof (struct ovsrec_bgp_route)),
            VTY_NEWLINE);

    count = bgp_get_peer_count(bgp_router_context);
    vty_out (vty, "Peers %ld, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                           count * sizeof (struct ovsrec_bgp_neighbor)),
             VTY_NEWLINE);

    count = bgp_get_peer_group_count(bgp_router_context);
    vty_out (vty, "Peer groups %ld, using %s of memory%s", count,
             mtype_memstr (memstrbuf, sizeof (memstrbuf),
                           count * sizeof (struct ovsrec_bgp_neighbor)),
             VTY_NEWLINE);
    ...
    if (i > 0)
    vty_out (vty, "%s    Total number of neighbors %d%s", VTY_NEWLINE,
             i, VTY_NEWLINE);
    else
