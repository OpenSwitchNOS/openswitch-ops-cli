...
neighbor_context_check(const struct ovsdb_idl_txn *txn, char *vrf_name, char *ip_addr) {
    const struct ovsrec_vrf *vrf_row;
    const struct ovsrec_bgp_router *bgp_router_context;
    const struct ovsrec_bgp_neighbor *ovs_bgp_neighbor;
...
cli_neighbor_aspath_filter_cmd_execute(char *vrf_name, char *ip_addr, char *name, char *dir)
{
    ...
    ovs_bgp_neighbor = neighbor_context_check(txn, vrf_name, ip_addr);
                                  ...
                                  const char *name, const char *action, const char *description)
{
    ...
    char **permit_entries;
    char **deny_entries;
    char *new_permit_entry[1];
    char *new_deny_entry[1];
    int num_entries;
    int i;
