                  ...
                  boolean print_header)
{
    ...
    if (ppsd->flags & BGP_INFO_STALE)
        vty_out (vty, ", (stale)");
    if (ppsd->aggregator_addr) {
        if (strlen(ppsd->aggregator_addr) != 0) {
            if (ppsd->aggregator_id != 0) {
                VLOG_INFO("HEMALI: aggregate aadr not null\n");
    ...
    vty_out (vty, "%s", VTY_NEWLINE);
    if(ppsd->community){
        VLOG_INFO("HEMALI: community not null");
        if (strlen(ppsd->community) != 0) {
