...
static int show_dhcp_range()
{
    ...
    OVSREC_DHCPSRV_RANGE_FOR_EACH(row, idl){
        ...
        if (row->prefix_len) {
            snprintf(prefix_len, 8, "%d", row->prefix_len[0]);
        } else {
            snprintf(prefix_len, 8, "%s", "*");
        }

        if (row->lease_duration) {
            snprintf(lease_duration, 16, "%d", row->lease_duration[0]);
        } else {
            snprintf(lease_duration, 16, "%s", "*");
        }

        vty_out(vty,"Name: %s\t Start_IP_Address : %s\t End_IP_Address : %s\t "
                    "Set_Tag : %s\t Prefix_Len : %s\t "
                    "Lease Duration in minutes: %s",
                     row->name, row->start_ip_address, row->end_ip_address,
                     row->set_tag, prefix_len, lease_duration);

        vty_out(vty,"\tMatch Tags :");
...
static int show_dhcp_host()
{
    ...
    OVSREC_DHCPSRV_STATIC_HOST_FOR_EACH(row, idl){
        ...
        vty_out(vty, "IP Address: %s\t Hostname : %s\t Client ID : %s "
                     "Lease duration in minutes: %s",
                      row->ip_address, row->client_hostname, row->client_id,
                      lease_duration);
        vty_out(vty,"\tMAC Addresses :");

        for (i = 0; i < row->n_mac_addresses; i++) {
            vty_out(vty,"%s ",row->mac_addresses[i]);
        }

        vty_out(vty,"\tSet Tags :");
...
static int show_dhcp_options()
{
    ...
    vty_out(vty,"%sDHCP OPTION %s",
                 VTY_NEWLINE, VTY_NEWLINE);
    vty_out(vty, "--------------%s",
                 VTY_NEWLINE);

    if (!row) {
        vty_out(vty, "No DHCP Options defined.%s", VTY_NEWLINE);
        return CMD_SUCCESS;
    }
    ...
    OVSREC_DHCPSRV_OPTION_FOR_EACH(row, idl){
        ...
        if (row->option_number) {
            snprintf(option_number, 8, "%d", row->option_number[0]);
        } else {
        ...
        vty_out(vty,"Option Number: %s\t Option Name : %s\t "
                    "Option Value : %s",
                     option_number, row->option_name ? row->option_name : "*",
...
static int show_dhcp_match()
{
    ...
    vty_out(vty, "%sDHCP MATCH %s",
                  VTY_NEWLINE,VTY_NEWLINE);
    vty_out(vty, "--------------%s",
                  VTY_NEWLINE);

    if (!row) {
        vty_out(vty, "No DHCP MATCH defined.%s", VTY_NEWLINE);
        return CMD_SUCCESS;
    }
    ...
    OVSREC_DHCPSRV_MATCH_FOR_EACH(row, idl){
        ...
        vty_out(vty, "Set Tag : %s\t,Option Number: %s\t Option Name : %s\t "
                     "Option Value : %s",
                      row->set_tag,
                      option_number,row->option_name ? row->option_name : "*",
                      row->option_value ? row->option_value : "*");
...
static int show_dhcp_bootp()
{
    ...
    vty_out(vty, "%sDHCP BOOTP %s",
                  VTY_NEWLINE,VTY_NEWLINE);
    vty_out(vty, "----------------%s",
                  VTY_NEWLINE);

    if (!row) {
        vty_out(vty, "No DHCP BOOTP defined.%s", VTY_NEWLINE);
        return CMD_SUCCESS;
    }

    SMAP_FOR_EACH(node,&row->bootp) {
        if (strcmp(node->key,"no_matching_tag") != 0){
            vty_out(vty, "Tag : %s  , File = %s%s",
                          node->key,node->value,VTY_NEWLINE);
        } else {
            vty_out(vty, "* (default file) : , File = %s%s",
...
static int show_tftp_server()
{
    ...
    buff = smap_get(&row->other_config,"tftp_server_enable");
    if (buff == NULL) {
        vty_out(vty, "TFTP-Server is not enabled.%s", VTY_NEWLINE);
        return CMD_SUCCESS;
    } else {
        vty_out(vty, "TFTP_Server_Enable : %s%s", buff, VTY_NEWLINE);
        buff = NULL;
        buff = smap_get(&row->other_config,"tftp_server_secure");
        if (buff == NULL) {
            vty_out(vty, "TFTP-Secure mode is not enabled.%s",
                          VTY_NEWLINE);
        } else {
            vty_out(vty, "TFTP_Server_Secure mode : %s%s",
                          buff, VTY_NEWLINE);
        }

        buff = smap_get(&row->other_config,"tftp_server_path");
        if (buff == NULL){
            vty_out(vty,"TFTP_Server_ Path  is not enabled.%s",VTY_NEWLINE);
            return CMD_SUCCESS;
        } else {
            vty_out(vty,"TFTP_Server_Path : %s%s",buff,VTY_NEWLINE);
        }
...
static struct ovsrec_vrf* dhcp_server_vrf_lookup(const char *vrf_name)
{
...
static int tftp_server_enable_disable(bool enable)
{
    ...
    if (!ovs_row) {
        vty_out(vty, "Error: Could not fetch Open VSwitch data.%s",
                     VTY_NEWLINE);
        VLOG_ERR("%s Open VSwitch table did not have any rows. Ideally it "
    ...
    ovsrec_open_vswitch_set_other_config(ovs_row, &smap);
...
static int tftp_server_secure_enable_disable(bool enable)
{
    ...
    ovs_row = ovsrec_open_vswitch_first(idl);
    if (!ovs_row) {
        vty_out(vty, "Error: Could not fetch default Open VSwitch data.%s",
                     VTY_NEWLINE);
        VLOG_ERR("%s Open VSwitch table did not have any rows. Ideally it "
                 "should have just one entry.", __func__);
             cli_do_config_abort(status_txn);
...
static int tftp_server_path_config(const char *path, bool add)
{
    ...
    smap_clone(&smap, &ovs_row->other_config);
    if (!add) {
        curr_path = smap_get(&smap, "tftp_server_path");
...
{
    const struct ovsrec_vrf *vrf_row = NULL;
    const struct ovsrec_dhcp_server *dhcp_server_row = NULL;
    struct smap smap = SMAP_INITIALIZER(&smap);
    struct ovsdb_idl_txn *status_txn = NULL;
    size_t n,i;
...
static int dhcp_server_delete_match(dhcp_srv_match_params_t *match_params)
{
    const struct ovsrec_vrf *vrf_row = NULL;
    const struct ovsrec_dhcp_server *dhcp_server_row = NULL;
    const struct ovsrec_dhcpsrv_match *dhcpsrv_match_row = NULL;
    const struct ovsrec_dhcpsrv_match *dhcpsrv_match_temp = NULL;
    struct ovsdb_idl_txn *status_txn = NULL;
    ...
    for (i = 0; i < dhcp_server_row->n_matches; i++) {
        ...
        if (strcmp(dhcpsrv_match_temp->set_tag, match_params->set_tag) == 0 &&
            ((match_params->option_number == -1 && dhcpsrv_match_temp->n_option_number == 0) ||
            (match_params->option_number != -1 && dhcpsrv_match_temp->n_option_number == 1 &&
            match_params->option_number == *(dhcpsrv_match_temp->option_number))) &&
            ((match_params->option_name == NULL && dhcpsrv_match_temp->option_name == NULL) ||
            (match_params->option_name != NULL && dhcpsrv_match_temp->option_name != NULL &&
            strcmp(match_params->option_name, dhcpsrv_match_temp->option_name) == 0)) &&
            ((match_params->option_value == NULL && dhcpsrv_match_temp->option_value == NULL) ||
            (match_params->option_value != NULL && dhcpsrv_match_temp->option_value != NULL &&
            strcmp(match_params->option_value, dhcpsrv_match_temp->option_value) == 0))) {
    ...
    d_match=xmalloc(sizeof *dhcp_server_row->matches * (dhcp_server_row->n_matches - 1));
    ...
    struct ovsrec_dhcp_server *temp_dhcp_server = CONST_CAST(struct ovsrec_dhcp_server*, dhcp_server_row);
...
static int dhcp_server_add_match(dhcp_srv_match_params_t *match_params)
{
    ...
    struct ovsrec_dhcpsrv_match *temp_dhcp_row = CONST_CAST(struct ovsrec_dhcpsrv_match*, dhcpsrv_match_row);
...
static int dhcp_server_delete_option(dhcp_srv_option_params_t *option_params)
{
    const struct ovsrec_vrf *vrf_row = NULL;
    const struct ovsrec_dhcp_server *dhcp_server_row = NULL;
    const struct ovsrec_dhcpsrv_option *dhcpsrv_option_row = NULL;
    const struct ovsrec_dhcpsrv_option *dhcpsrv_option_temp = NULL;
    struct ovsdb_idl_txn *status_txn = NULL;
    ...
    for (i = 0; i < dhcp_server_row->n_dhcp_options; i++) {
        if (dhcp_server_row->dhcp_options[i]->n_match_tags == dhcpsrv_option_temp->n_match_tags &&
            strcmp(dhcp_server_row->dhcp_options[i]->option_value, dhcpsrv_option_temp->option_value) == 0 &&
            dhcp_server_row->dhcp_options[i]->n_option_number == dhcpsrv_option_temp->n_option_number &&
            (dhcpsrv_option_temp->n_option_number == 0 || (*(dhcp_server_row->dhcp_options[i]->option_number) ==
             *(dhcpsrv_option_temp->option_number))) &&
            ((dhcp_server_row->dhcp_options[i]->option_name == NULL && dhcpsrv_option_temp->option_name == NULL) ||
             (dhcp_server_row->dhcp_options[i]->option_name != NULL && dhcpsrv_option_temp->option_name != NULL &&
            strcmp(dhcp_server_row->dhcp_options[i]->option_name, dhcpsrv_option_temp->option_name) == 0))) {
            entry_found = 1;
            for (j = 0 ; j < dhcpsrv_option_temp->n_match_tags; j++) {
                if (strcmp(dhcp_server_row->dhcp_options[i]->match_tags[j], dhcpsrv_option_temp->match_tags[j]) != 0) {
                    entry_found = 0;
                    break;
                }
            }
    ...
    d_option = xmalloc(sizeof *dhcp_server_row->dhcp_options * (dhcp_server_row->n_dhcp_options - 1));
...
static int dhcp_server_add_option(dhcp_srv_option_params_t *option_params)
{
    ...
    for (i = 0; i < dhcp_server_row->n_dhcp_options; i++) {
        if (dhcp_server_row->dhcp_options[i]->n_match_tags == dhcpsrv_option_row->n_match_tags &&
            strcmp(dhcp_server_row->dhcp_options[i]->option_value, dhcpsrv_option_row->option_value) == 0 &&
            dhcp_server_row->dhcp_options[i]->n_option_number == dhcpsrv_option_row->n_option_number &&
            (dhcpsrv_option_row->n_option_number == 0 || (*(dhcp_server_row->dhcp_options[i]->option_number) ==
             *(dhcpsrv_option_row->option_number))) &&
            ((dhcp_server_row->dhcp_options[i]->option_name == NULL && dhcpsrv_option_row->option_name == NULL) ||
             (dhcp_server_row->dhcp_options[i]->option_name != NULL && dhcpsrv_option_row->option_name != NULL &&
            strcmp(dhcp_server_row->dhcp_options[i]->option_name, dhcpsrv_option_row->option_name) == 0))) {
            duplicate_entry = 1;
            for (j = 0 ; j < dhcpsrv_option_row->n_match_tags; j++) {
                if (strcmp(dhcp_server_row->dhcp_options[i]->match_tags[j], dhcpsrv_option_row->match_tags[j]) != 0) {
                    duplicate_entry = 0;
                    break;
                }
            }
    ...
    struct ovsrec_dhcpsrv_option *temp_dhcp_row = CONST_CAST(struct ovsrec_dhcpsrv_option*, dhcpsrv_option_row);
...
static int dhcp_server_add_static_host(dhcp_srv_static_host_params_t *static_host_params)
{
    const struct ovsrec_vrf *vrf_row = NULL;
    const struct ovsrec_dhcp_server *dhcp_server_row = NULL;
    const struct ovsrec_dhcpsrv_static_host *dhcpsrv_static_host_row = NULL;
    const struct ovsrec_dhcpsrv_static_host *dhcpsrv_static_host_temp = NULL;
    struct ovsdb_idl_txn *status_txn = NULL;
    ...
    d_static_host=xmalloc(sizeof *dhcp_server_row->static_hosts * (dhcp_server_row->n_static_hosts + 1));
    for (i = 0; i < dhcp_server_row->n_static_hosts; i++) {
        d_static_host[i] = dhcp_server_row->static_hosts[i];
    }

    struct ovsrec_dhcpsrv_static_host *temp_dhcp_row = CONST_CAST(struct ovsrec_dhcpsrv_static_host*, dhcpsrv_static_host_row);
    ...
    struct ovsrec_dhcp_server *temp_dhcp_server = CONST_CAST(struct ovsrec_dhcp_server*,
                                                  dhcp_server_row);
    ovsrec_vrf_set_dhcp_server(vrf_row, temp_dhcp_server);
    free(d_static_host);
...
static int dhcp_server_delete_static_host(dhcp_srv_static_host_params_t *static_host_params)
{
    const struct ovsrec_vrf *vrf_row = NULL;
    const struct ovsrec_dhcp_server *dhcp_server_row = NULL;
    const struct ovsrec_dhcpsrv_static_host *dhcpsrv_static_host_row = NULL;
...
static int dhcp_server_delete_range(dhcp_srv_range_params_t *range_params)
{
    const struct ovsrec_vrf *vrf_row = NULL;
    const struct ovsrec_dhcp_server *dhcp_server_row = NULL;
    const struct ovsrec_dhcpsrv_range *dhcpsrv_range_row = NULL;
    const struct ovsrec_dhcpsrv_range *dhcpsrv_range_temp = NULL;
    struct ovsdb_idl_txn *status_txn = NULL;
    ...
    d_range=xmalloc(sizeof *dhcp_server_row->ranges * (dhcp_server_row->n_ranges - 1));
...
static int dhcp_server_add_range(dhcp_srv_range_params_t *range_params)
{
    ...
    struct ovsrec_dhcpsrv_range *temp_dhcp_row = CONST_CAST(struct ovsrec_dhcpsrv_range*, dhcpsrv_range_row);
...
DEFUN (cli_dhcp_server_match_number_delete,
       ...
       "no match set tag TAG match-option-number WORD {match-option-value WORD}",
       "Remove DHCP server configuration\n"
       "DHCP match configuration\n"
       "Set tag\n"
       "Set tag\n"
...
DEFUN (cli_dhcp_server_match_number_add,
       ...
       "match set tag TAG match-option-number <0-255> {match-option-value WORD}",
       "DHCP match configuration\n"
       "Set tag\n"
       "Set tag\n"
       "Enter tag name\n"
...
DEFUN (cli_dhcp_server_option_name_delete,
       ...
       "no option set option-name WORD option-value WORD {match tags TAGS | ipv6}",
       "Remove DHCP server configuration\n"
...
DEFUN (cli_dhcp_server_option_number_delete,
       ...
       "no option set option-number <0-255> option-value WORD {match tags TAGS | ipv6}",
       "Remove DHCP server configuration\n"
       "DHCP Options configuration\n"
       "Set option-number/option-name\n"
       "Set option number\n"
...
DEFUN (cli_dhcp_server_option_name_add,
       ...
       "option set option-name WORD option-value WORD {match tags TAGS | ipv6}",
       "DHCP Options configuration\n"
       "Set option-number/option-name\n"
       "Set option name\n"
       "Enter option name\n"
...
DEFUN (cli_dhcp_server_option_number_add,
       ...
       "option set option-number <0-255> option-value WORD {match tags TAGS | ipv6}",
       "DHCP Options configuration\n"
       "Set option-number/option-name\n"
       "Set option number\n"
       "Enter option number\n"
...
DEFUN (cli_dhcp_server_static_host_delete,
       ...
       "no static A.B.C.D "
       "{match-mac-addresses MAC-ADDRs | set tags TAGs | match-client-hostname WORD | "
       "match-client-id WORD | lease-duration LEASE-TIME}",
       "Remove DHCP server configuration\n"
       "Static leases configuration\n"
       "Enter host static IP address\n"
...
DEFUN (cli_dhcp_server_range_delete,
       ...
       "no range WORD start-ip-address A.B.C.D "
       "{static | end-ip-address A.B.C.D | set tag TAG-NAME | match tags TAGS | "
       "netmask A.B.C.D | broadcast A.B.C.D | prefix-len <64-128> | lease-duration <0-65535>}",
       "Remove DHCP server configuration\n"
       "Remove DHCP server IP address range configuration\n"
       "Enter DHCP server IP address range name to be removed\n"
       "Start IP address\n"
       ...
       "Enter DHCP lease duration in minutes, 0 for \"infinite\"\n")
{
    ...
    if (argv[8] == NULL) {
        ...
        range_params.prefix_len = atoi(argv[8]);
    }

    if (argv[9] == NULL) {
        range_params.lease_duration = 60;
    } else {
        range_params.lease_duration = atoi(argv[9]);
    }
...
DEFUN (cli_dhcp_server_range_add,
       ...
       "range WORD start-ip-address A.B.C.D "
       "{static | end-ip-address A.B.C.D | set tag TAG-NAME | match tags TAGS | "
       "netmask A.B.C.D | broadcast A.B.C.D | prefix-len <64-128> | lease-duration <0-65535>}",
...
dhcp_tftp_vty_init (void)
{
    ...
    install_element(DHCP_SERVER_NODE, &cli_dhcp_server_static_host_delete_cmd);
    ...
    install_element(DHCP_SERVER_NODE, &cli_dhcp_server_option_number_delete_cmd);
    ...
    install_element(DHCP_SERVER_NODE, &cli_dhcp_server_match_number_delete_cmd);
