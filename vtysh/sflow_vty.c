...
/* This function displays the sflow configuration set in the database */
static int sflow_show(void)
{
  ...
  if (!sflow_row)
    {
      vty_out(vty, "sFlow not yet configured.%s",
                     VTY_NEWLINE);
  ...
  if(sflow_row->sampling != NULL)
    ...
    vty_out(vty, "Sampling Rate                 %d%s",
            OVSDB_SFLOW_SAMPLING_RATE, VTY_NEWLINE);
  ...
  if(sflow_row->polling != NULL)
    ...
    vty_out(vty, "Polling Interval              %d%s",
            OVSDB_SFLOW_POLLING_INTERVAL, VTY_NEWLINE);
  ...
  if(sflow_row->header != NULL)
    ...
    vty_out(vty, "Header Size                   %d%s",
            OVSDB_SFLOW_HEADER_SIZE, VTY_NEWLINE);
  ...
  if(sflow_row->max_datagram != NULL)
    ...
    vty_out(vty, "Max Datagram Size             %d%s",
            OVSDB_SFLOW_MAX_DATAGRAM_SIZE, VTY_NEWLINE);
...
/* This function sets/unsets the row name in the database.
 * As of now, we provide support for setting  sflow only at the global level*/
                               ...
                               const char *vrf, bool set)
{
  ...
  if (port == NULL && vrf == NULL)
    {
      snprintf(cmd_str, MAX_CMD_LEN,"%s\\%s\\%s", ip, OVSDB_SFLOW_DFLT_PORT,
               DEFAULT_VRF_NAME);
    }
...
{
    return sflow_set_global_status(1);
}
...
DEFUN (cli_sflow_no_set_global_status,
       ...
       "Enable/Disable sflow feature\n")
{
    return sflow_set_global_status(0);
}
...
{
    return sflow_set_collector(argv[0], argv[1], argv[2], 1);
}
...
{
    return sflow_set_collector(argv[0], argv[1], argv[2], 0);
}
