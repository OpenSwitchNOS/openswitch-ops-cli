  ...
  OVSREC_INTERFACE_FOR_EACH (ifrow, idl)
    {
      if(0 == strcmp(interface, ifrow->name))
      {
        ...
        if (value)
        {
          if (strcmp(smap_get(&port_row->other_config,
                                PORT_OTHER_CONFIG_SFLOW_PER_INTERFACE_KEY_STR),
...
sflow_set_port_config(bool set)
{
  ...
  value = set ? PORT_OTHER_CONFIG_SFLOW_PER_INTERFACE_VALUE_TRUE : PORT_OTHER_CONFIG_SFLOW_PER_INTERFACE_VALUE_FALSE;

  if (smap_is_empty(&port_row->other_config))
    {
      smap_add(&other_config, PORT_OTHER_CONFIG_SFLOW_PER_INTERFACE_KEY_STR, value);
    } else
    {
      smap_clone(&other_config, &port_row->other_config);
      smap_replace(&other_config, PORT_OTHER_CONFIG_SFLOW_PER_INTERFACE_KEY_STR, value);
    }
