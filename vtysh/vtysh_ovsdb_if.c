...
{
    const struct ovsrec_source_repository *src_repo_row = NULL;

    OVSREC_SOURCE_REPOSITORY_FOR_EACH(src_repo_row, idl) {
        if (src_repo_row) {
            vty_out(vty, "PACKAGE    : %-128s\n",  src_repo_row->name);
            vty_out(vty, "VERSION    : %-128s\n", (src_repo_row->version[0] == '\0') ? "Not Available" : src_repo_row->version);
            vty_out(vty, "SOURCE URI : %-128s\n\n", (src_repo_row->src_uri[0] == '\0') ? "Not Available" : src_repo_row->src_uri);
        }
    }
}
