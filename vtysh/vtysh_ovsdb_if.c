...
unsigned char ospf_vlink_count[OVS_OSPF_VL_MAX_COUNT] = {0};
...
vrf_ovsdb_init()
{
    ...
    struct ovsdb_idl_index *vrf_index = NULL;
...
vtysh_ovsdb_banner_set(const char* in, enum banner_type which_banner)
{
    ...
    if (ovs) {
        ...
        VLOG_ERR(VLOG_BANNER_OVS_SYSTEM_FAIL_SET);
        return CMD_OVSDB_FAILURE;
    }
}
...
vtysh_ovsdb_banner_exec_get(char* banner)
{
    ...
    if (ovs) {
        ...
        VLOG_ERR(VLOG_BANNER_OVS_SYSTEM_FAIL_GET);
        return CMD_OVSDB_FAILURE;
    }
    return CMD_SUCCESS;
}
