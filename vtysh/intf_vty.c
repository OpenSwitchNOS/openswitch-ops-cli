        ...
        if (strncmp(port_row->name, LAG_PORT_NAME_PREFIX, LAG_PORT_NAME_PREFIX_LENGTH) == 0)
        {
            for (k = 0; k < port_row->n_interfaces; k++)
            {
                ...
                if(strcmp(if_name, if_row->name) == 0)
                {
                    ...
                    vty_out(vty, "%3s%s %d%s", "", "lag",
                            atoi(&port_row->name[LAG_PORT_NAME_PREFIX_LENGTH]), VTY_NEWLINE);
                }
            }
        }
    }
                                                ...
                                                struct vty *vty, bool *bPrinted)
{
    ...
    if (data)
    {
        ...
        vty_out (vty, "%3s%s %d%s", "", "lacp port-id", atoi(data), VTY_NEWLINE);
    }
    ...
    if (data)
    {
        if (!(*bPrinted))
        {
            *bPrinted = true;
            ...
            vty_out (vty, "%3s%s %d%s", "", "lacp port-priority", atoi(data), VTY_NEWLINE);
...
parse_lag(struct vty *vty)
{
    ...
    OVSREC_PORT_FOR_EACH(port_row, idl)
    {
        if(strncmp(port_row->name, LAG_PORT_NAME_PREFIX, LAG_PORT_NAME_PREFIX_LENGTH) == 0)
        {
            ...
            vty_out (vty, "interface lag %d%s", atoi(&port_row->name[LAG_PORT_NAME_PREFIX_LENGTH]), VTY_NEWLINE);
            ...
            if(data)
            {
                ...
                {
                  vty_out (vty, "%3slacp fallback%s%s"," ", VTY_NEWLINE);
                }
            }
