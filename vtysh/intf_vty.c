      ...
      "Configure QSFP interface for 4x 10Gb operation or 1x 40Gb operation.\n")
{
   ...
   OVSREC_INTERFACE_FOR_EACH(row, idl)
   {
      if (strcmp(row->name, (char*)vty->index) == 0)
      {
          /* if not splittable, error */
          const char *split_value = NULL;

          split_value = smap_get(&row->hw_intf_info,
                                 INTERFACE_HW_INTF_INFO_MAP_SPLIT_4);
          if ((split_value == NULL) ||
              (strcmp(split_value, INTERFACE_HW_INTF_INFO_MAP_SPLIT_4_TRUE) != 0))
          {
              VLOG_ERR("split only applies to QSFP interfaces with split capability");
              cli_do_config_abort(status_txn);
              return CMD_ERR_INVALID;
          }
         if (vty_flags & CMD_FLAG_NO_CMD)
         {
            smap_remove(&row->user_config, INTERFACE_USER_CONFIG_MAP_LANE_SPLIT);
         }
         else
         {
            smap_replace(&row->user_config, INTERFACE_USER_CONFIG_MAP_LANE_SPLIT,
   ...
   OVSREC_INTERFACE_FOR_EACH(ifrow, idl)
   {
      ...
      else
      {
              ...
              strcmp(cur_state, INTERFACE_HW_INTF_INFO_MAP_PLUGGABLE_TRUE) == 0)
         {
            ...
            vty_out(vty, " Pluggable transceiver info:%s", VTY_NEWLINE);
            SMAP_FOR_EACH(node, &ifrow->pm_info)
            {
                vty_out(vty, "   %s = %s%s", node->key, node->value, VTY_NEWLINE);
            }
         }
