 ...
 */
const struct
ovsrec_vrf* port_match_in_vrf(const struct ovsrec_port *port_row)
{
...
*/
const struct
ovsrec_port* port_find(const char *if_name)
{
        ...
        int flags, int argc, const char *argv[])
{
    ...
    OVSREC_INTERFACE_FOR_EACH(row, idl)
    {
        ...
        {
            PRINT_INT_HEADER_IN_SHOW_RUN
                vty_out(vty, "   autonegotiation %s %s",
                        cur_state, VTY_NEWLINE);
        }
...
create_vlan_interface(const char *vlan_if)
{
    ...
    /* If both port and interface exists
       return success nothing to change here */
    ...
    if (status == TXN_SUCCESS) {
        ...
        VLOG_DBG("%s The command resulted in no change "
