...
vtysh_sftp_server_context_clientcallback (void *p_private)
{
   ...
   sftp_status = (char *)smap_get(&row->aaa, SFTP_SERVER_CONFIG);
   if (sftp_status && !strcmp(sftp_status, SFTP_SERVER_ENABLE)) {
...
vtysh_init_sftp_context_clients (void)
{
   ...
   if (e_vtysh_ok != retval)
   {
     vtysh_ovsdb_config_logmsg(VTYSH_OVSDB_CONFIG_ERR,
                        "SFTP server context unable to add config callback");
     assert(0);
     return retval;
   }
   return e_vtysh_ok;
