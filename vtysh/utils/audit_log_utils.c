 ...
 *     This file has definition of audit_log_usr_message wrapper function.
 ***************************************************************************/
...
/*-----------------------------------------------------------------------------
  ...
  | Parameters :
  |     char *op                      : String representing the operation being
  |                                     performed
  ...
  -----------------------------------------------------------------------------*/
void
audit_log_user_msg(char *op, const char *cfgdata, char *hostname, int result)
{
    ...
    if (cfgdata != NULL){
       ...
       if (cfg != NULL){
           strncat(strncat(aubuf,
                          cfg,
                          (MAX_CFGDATA_LEN - (MAX_OP_DESC_LEN + MAX_RUSER_NAME_LEN + MAX_RUSER_ENV_LEN + 1))),
                   " ",
                   strlen(" "));
           free(cfg);
       }
    ...
    if (remote_user != NULL) {
        ...
        snprintf(user_name, MAX_RUSER_NAME_LEN + MAX_RUSER_ENV_LEN, "user=%s", remote_user);
        strncat(aubuf, user_name, MAX_RUSER_NAME_LEN + MAX_RUSER_ENV_LEN);
    }
