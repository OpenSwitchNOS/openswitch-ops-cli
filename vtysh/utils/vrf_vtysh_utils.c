...
static int
compare_digit_and_name (const char *name1, const char *name2)
{
    if (isdigit(*name1))
        return -1;
    else
...
int compare_both_numbers(const char *name1, const char *name2)
{
    uint i1 = 0, i2 = 0;
...
| Responsibility : Custom comparator function used for comparing two VRF element
| Parameters:
...
{
    const struct ovsrec_vrf *a = (struct ovsrec_vrf *)a_;
    const struct ovsrec_vrf *b = (struct ovsrec_vrf *)b_;
    char *name1 = (a)->name;
    char *name2 = (b)->name;

    if (isdigit(*name1) && isdigit(*name2))
    {
         return compare_both_numbers (name1, name2);
    }
    /* Case 2 */
    else if (isdigit(*name1) || isdigit(*name2))
    {
        return compare_digit_and_name (name1, name2);
    }
    /* Case 3 */
    else
    {
...
/*-----------------------------------------------------------------------------
| Function       : compare_interface_nodes_vrf
| Responsibility : the function is used for comparing two interface elements
| Parameters:
|    *a_: Pointer to first interface element
|    *b_: Pointer to second interface element
| Return:
|    int: zero if two elements are equal, 1 if the first elemnt is
|         bigger than second, -1 if the first element is smaller than second
| sort comparator function.
...
-----------------------------------------------------------------------------*/
int
compare_interface_nodes_vrf (const void *a_, const void *b_)
{
    ...
    {
        return compare_digit_and_name (name_intf1, name_intf2);
    }
    /* Case 3 */
    else
    {
