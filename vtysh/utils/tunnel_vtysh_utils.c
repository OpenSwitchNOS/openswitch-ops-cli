                ...
                const char *new_ip)
{
    ...
    return set_intf_option_ip(if_row, OVSREC_INTERFACE_OPTIONS_TUNNEL_SOURCE_IP,
                              new_ip);
}

int
set_intf_option_ip(const struct ovsrec_interface *if_row, const char *option,
                   const char *new_ip)
{
    enum ovsdb_idl_txn_status txn_status;

    // Update the IP if it is different
    const char *curr_ip = smap_get(&if_row->options, option);

    // IP exists and is the same.. Skip configuration
    if (curr_ip && !strcmp(new_ip, curr_ip))
    {
        VLOG_DBG("Skip configuration since IPs are identical.");
        return CMD_SUCCESS;
    }
    ...
    // Clone any existing options and update the tunnel source IP
    struct smap if_options;
    smap_clone(&if_options, &if_row->options);
    smap_replace(&if_options, option, new_ip);
