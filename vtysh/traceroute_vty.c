    ...
    switch(type)
    {
        case DESTINATION:
        {
            ...
            if (isalpha((int) *value))
            {
                ...
                {
                    vty_out (vty, "Invalid Hostname : hostname should be less than 256 characters %s",VTY_NEWLINE);
                    return CMD_WARNING;
                }
                p->isIpv4 = true;
...
DEFUN (cli_traceroute6,
       cli_traceroute6_cmd,
    "traceroute6 ( X:X::X:X | WORD ) { dstport <1-34000> | maxttl <1-255> | probes <1-5>| timeout <1-120>} ",
    TRACEROUTE_STR
    TRACEROUTE_IP
    TRACEROUTE_HOST
    TRACEROUTE_DSTPORT
