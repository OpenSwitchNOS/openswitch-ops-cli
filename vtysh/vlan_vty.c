    ...
    "The VLAN identifier\n")
{
    ...
    {
        vty_out(vty, "Interface is in access mode.%s", VTY_NEWLINE);
        cli_do_config_abort(status_txn);
        return CMD_SUCCESS;
    }
    ...
    "The VLAN identifier\n")
{
    ...
    if (vlan_port_row->vlan_mode != NULL &&
        ...
        strcmp(vlan_port_row->vlan_mode, OVSREC_PORT_VLAN_MODE_NATIVE_UNTAGGED) != 0)
    {
        vty_out(vty, "Interface is not in trunk mode.%s", VTY_NEWLINE);
        cli_do_config_abort(status_txn);
        return CMD_SUCCESS;
    }
    ...
    "The VLAN identifier\n")
{
    ...
    if (vlan_port_row->vlan_mode != NULL &&
        strcmp(vlan_port_row->vlan_mode, OVSREC_PORT_VLAN_MODE_NATIVE_TAGGED) != 0)
    {
        vty_out(vty, "Interface is not in native-tagged mode.%s", VTY_NEWLINE);
        cli_do_config_abort(status_txn);
        return CMD_SUCCESS;
    }
