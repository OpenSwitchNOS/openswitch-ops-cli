...
static int
ip_route_common(struct vty *vty, char **argv, char *metric)
{
...
DEFUN (vtysh_ip_route_metric,
       vtysh_ip_route_metric_cmd,
...
static int
no_ip_route_common(struct vty *vty, char **argv, char *metric)
{
    int ret;
    const struct ovsrec_route *row_route = NULL;
    int flag = 0;
        ...
        if (row_route->prefix != NULL) {
            ...
            if (0 == strcmp(prefix_str,row_route->prefix )) {
                if (row_route->n_nexthops) {
                    ...
                    if (metric != NULL) {
                        (atoi(metric) == *row_route->distance) ? (distance_match = 1)
                            : (distance_match = 0);
                    } else {
                        distance_match = 1;
                    }
...
DEFUN (vtysh_no_ip_route_metric,
       vtysh_no_ip_route_metric_cmd,
...
static int
ipv6_route_common(struct vty *vty, char **argv, char *metric)
{
    const struct ovsrec_route *row = NULL;
...
DEFUN (vtysh_ipv6_route_metric,
       vtysh_ipv6_route_metric_cmd,
...
static int
no_ipv6_route_common(struct vty *vty, char **argv, char *metric)
{
    int ret;
    const struct ovsrec_route *row_route = NULL;
    int flag = 0;
...
DEFUN (vtysh_no_ipv6_route_metric,
       vtysh_no_ipv6_route_metric_cmd,
