                   ...
                   const struct ovsrec_route * row, char * ip_addr_family)
{
  ...
  int k;
  int64_t distance;
  char ip4_addrcopy[256];
  char *ipv4_address = NULL;
  char ip6_addrcopy[256];
      ...
      OVSREC_PORT_FOR_EACH (row_port, idl)
        {
          ...
          for (k = 0; k < row_port->n_ip4_address_secondary; k++)
            {
              if (row_port->ip4_address_secondary[k])
                {
                  memset(ip4_addrcopy, 0, sizeof(ip4_addrcopy));
                  strcpy(ip4_addrcopy, row_port->ip4_address_secondary[k]);
                  ipv4_address = strtok(ip4_addrcopy, "/");
                  if (!strcmp (ipv4_address, nh_entry))
                    {
          ...
          for (k = 0; k < row_port->n_ip6_address_secondary; k++)
            {
              if (row_port->ip6_address_secondary[k])
                {
                  memset(ip6_addrcopy, 0, sizeof(ip6_addrcopy));
                  strcpy(ip6_addrcopy, row_port->ip6_address_secondary[k]);
                  ipv6_address = strtok(ip6_addrcopy, "/");
                  if (!strcmp (ipv6_address, nh_entry))
                    {
