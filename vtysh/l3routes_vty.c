 ...
 */
const struct ovsrec_vrf* port_in_vrf(const struct ovsrec_port *port_row)
{
        ...
        if (row->prefix != NULL) {
            if (!strcmp(row->prefix, argv[0]) && !strcmp(row->from, OVSREC_ROUTE_FROM_STATIC)) {
                if (row->n_nexthops > MAX_NEXTHOPS_PER_ROUTE - 1) {
                    vty_out(vty, "Maximum supported nexthops for a route are 32");
                    cli_do_config_abort(status_txn);
                    return CMD_OVSDB_FAILURE;
                }
                prefix_match = true;
