 ...
 * A.B.C.D/M */
static bool
ip4_address_is_invalid (char * ipaddr, bool is_nexthop)
{
 ...
 * X:X::X:X/M */
static bool
ip6_address_is_invalid (char * ipv6addr, bool is_nexthop)
{
                   ...
                   const struct ovsrec_route * row, char * ip_addr_family)
{
  ...
  /* Validating and assigning nexthop as an ip address */
  else
    {
      ...
      if (ip_addr_family_is_ipv4 && ip4_address_is_invalid (nh_entry, true))
        return NULL;

      /* Validating ipv6 addresses as nexthop */
      else if (ip_addr_family_is_ipv6 &&
               ip6_address_is_invalid (nh_entry, true))
        return NULL;
...
#endif
{
  ...
  if (ip4_address_is_invalid (prefix_str, false))
    {
      cli_do_config_abort (status_txn);
      return CMD_OVSDB_FAILURE;
    }
...
ipv6_route_common (struct vty *vty, char **argv, char *distance)
{
  ...
  if (ip6_address_is_invalid (prefix_str, false))
    {
      cli_do_config_abort (status_txn);
      return CMD_OVSDB_FAILURE;
    }
