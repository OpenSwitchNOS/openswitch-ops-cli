...
main (int argc, char **argv, char **env)
{
  ...
  pw = getpwuid( getuid());
  if(pw && !(rbac_check_user_permission(pw->pw_name,RBAC_READ_SWITCH_CONFIG) || (rbac_check_user_permission(pw->pw_name,RBAC_WRITE_SWITCH_CONFIG))))
  {
    fprintf(stderr, "%s does not have the required permissions to access the Vtysh shell.\n",pw->pw_name);
    exit(1);
  }
#ifdef ENABLE_OVSDB
