...
lacp_show_interfaces_all()
{
   ...
   OVSREC_PORT_FOR_EACH(lag_port, idl)
   {
      if(strncmp(lag_port->name, LAG_PORT_NAME_PREFIX, LAG_PORT_NAME_PREFIX_LENGTH) == 0)
      {
         ...
         {
            if_row = lag_port->interfaces[k];
            parse_state_from_db(smap_get(&if_row->lacp_status, INTERFACE_LACP_STATUS_MAP_ACTOR_STATE), lacp_state_ovsdb);
   ...
   OVSREC_PORT_FOR_EACH(lag_port, idl)
   {
      if(strncmp(lag_port->name, LAG_PORT_NAME_PREFIX, LAG_PORT_NAME_PREFIX_LENGTH) == 0)
      {
         ...
         {
            if_row = lag_port->interfaces[k];
            parse_state_from_db(smap_get(&if_row->lacp_status, INTERFACE_LACP_STATUS_MAP_PARTNER_STATE), lacp_state_ovsdb);
            lacp_state = get_lacp_state(lacp_state_ovsdb);
   ...
   OVSREC_PORT_FOR_EACH(port_row, idl)
   {
     if (strncmp(port_row->name, LAG_PORT_NAME_PREFIX, LAG_PORT_NAME_PREFIX_LENGTH) == 0)
     {
        for (k = 0; k < port_row->n_interfaces; k++)
        {
           ...
           if(strcmp(if_name, if_row->name) == 0)
           {
             parse_state_from_db(smap_get(&if_row->lacp_status, INTERFACE_LACP_STATUS_MAP_ACTOR_STATE), a_lacp_state_ovsdb);
             a_lacp_state = strdup(get_lacp_state(a_lacp_state_ovsdb)); /*get_lacp_state() returns a static char*, unless copied it will be overwritten by partner state*/
