...
exit_loop:
   ...
   /* Add Aggregation-Key */
   smap_clone(&smap, &interface_row->other_config);
   smap_replace(&smap, INTERFACE_OTHER_CONFIG_MAP_LACP_AGGREGATION_KEY,
                lag_number);
   ovsrec_interface_set_other_config(interface_row, &smap);
...
lacp_show_interfaces_all()
{
   ...
   const char *key = NULL, *agg_key = NULL;
   const char *port_priority = NULL, *port_id = NULL;
   ...
   OVSREC_PORT_FOR_EACH(lag_port, idl)
   {
      if(strncmp(lag_port->name, LAG_PORT_NAME_PREFIX, LAG_PORT_NAME_PREFIX_LENGTH) == 0)
      {
         ...
         {
            if_row = lag_port->interfaces[k];
            port_id = port_priority = key = agg_key = NULL;
            lacp_state = system_id = system_priority = NULL;
            if (data_in_db = smap_get(&if_row->lacp_status,
                             INTERFACE_LACP_STATUS_MAP_ACTOR_STATE))
            {
              if (parse_state_from_db(data_in_db, lacp_state_ovsdb) ==
                  LACP_STATUS_FIELD_COUNT)
              {
                lacp_state = get_lacp_state(lacp_state_ovsdb);
              }
              key = smap_get(&if_row->lacp_status,
                INTERFACE_LACP_STATUS_MAP_ACTOR_KEY);
              ...
              */
              ...
              parse_id_from_db(system_priority_id_ovsdb, &system_priority,
                &system_id);
            }
   ...
   vty_out(vty, delimiter);
   vty_out(vty,"%s", VTY_NEWLINE);
   vty_out(vty, columns, "Intf", "Aggregate", "Partner", "Port", "Key", "State",
           "System-id", "System", "Aggr");
   vty_out(vty,"%s", VTY_NEWLINE);
   ...
   OVSREC_PORT_FOR_EACH(lag_port, idl)
   {
      if(strncmp(lag_port->name, LAG_PORT_NAME_PREFIX, LAG_PORT_NAME_PREFIX_LENGTH) == 0)
      {
         ...
         {
            if_row = lag_port->interfaces[k];
            port_id = port_priority = key = NULL;
            agg_key = lacp_state = system_id = system_priority = NULL;
            if (data_in_db = smap_get(&if_row->lacp_status,
                INTERFACE_LACP_STATUS_MAP_PARTNER_STATE))
            {
              if (parse_state_from_db(data_in_db, lacp_state_ovsdb) ==
                  LACP_STATUS_FIELD_COUNT)
              {
