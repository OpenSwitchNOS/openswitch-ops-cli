...
get_lacp_state(const char *state)
{
   ...
   ret_state[n++] = state[4]? 'C': ' ';
   ret_state[n++] = state[5]? 'D': ' ';
   ret_state[n++] = state[6]? 'E': ' ';
   ret_state[n++] = state[7]? 'X': ' ';
...
{
 return sscanf(str, "%*[^01]%d%*[^01]%d%*[^01]%d%*[^01]%d%*[^01]%d%*[^01]%d%*[^01]%d%*[^01]%d", &ret_str[0],
                                                                                                &ret_str[1],
                                                                                                &ret_str[2],
                                                                                                &ret_str[3],
                                                                                                &ret_str[4],
   ...
   OVSREC_PORT_FOR_EACH(lag_port, idl)
   {
      if(strncmp(lag_port->name, LAG_PORT_NAME_PREFIX, LAG_PORT_NAME_PREFIX_LENGTH) == 0)
      {
         ...
         for (k = 0; k < lag_port->n_interfaces; k++)
         {
            ...
            {
              if (parse_state_from_db(data_in_db, lacp_state_ovsdb) == LACP_STATUS_FIELD_COUNT) lacp_state = get_lacp_state(lacp_state_ovsdb);
