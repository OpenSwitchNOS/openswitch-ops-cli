...
*/
static
int mask_ip4_subnet(const char* ip4)
{
    unsigned int i,j;
 ...
 * Argument-1 : sub-intf name
 * Argument-2 : ipv4 address
 * On SUCCESS function returns CMD_SUCCESS
 * On FAILURE function returns CMD_OVSDB_FAILURE
 */
static int
        ...
        "Delete Sub Interface IP address\n")
{
    ...
    {
        VLOG_DBG ("%s No IPv4 address configured on interface \"%s\".",
                __func__, if_name);
        return CMD_SUCCESS;
    }
 ...
 * Argument 1 : sub-intf name to be created
 * On Success this function return CMD_SUCCESS
