...
static int radius_server_add_host(const char *ipv4)
{
  ...
  if (!IS_VALID_IPV4(htonl(addr.s_addr))) {
      vty_out(vty, "Broadcast, multicast and loopback addresses are not allowed.%s",VTY_NEWLINE);
      cli_do_config_abort(status_txn);
      return CMD_ERR_NOTHING_TODO;
  }
...
static int radius_server_remove_auth_port(const char *ipv4, const char *authport)
{
  ...
  if (!tempRow) {
      vty_out(vty, "No radius server configured with IP %s %s",ipv4, VTY_NEWLINE);
      VLOG_ERR(OVSDB_ROW_FETCH_ERROR);
      cli_do_config_abort(status_txn);
      return CMD_ERR_NOTHING_TODO;
  }
