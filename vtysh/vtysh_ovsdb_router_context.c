  ...
  /* To consider all router entries. */
    OVSREC_BGP_ROUTER_FOR_EACH(bgp_router_context, p_msg->idl)
    {
        ...
        for (n_neighbors = 0; n_neighbors<bgp_router_context->n_bgp_neighbors;
            ...
            neighbor =   (char *)malloc(strlen(bgp_router_context->
                                    key_bgp_neighbors[n_neighbors]));
            strcpy(neighbor,bgp_router_context->key_bgp_neighbors[n_neighbors]);
            ...
            if (nbr_table->
                n_ebgp_multihop)
                vtysh_ovsdb_cli_print(p_msg,"%4s %s %s %s", "", "neighbor",bgp_router_context->
                                      key_bgp_neighbors[n_neighbors],
                                      "ebgp-multihop");
