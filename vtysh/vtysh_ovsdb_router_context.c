    ...
    for (i = 0; i < router_row->n_areas; i++)
    {
        ...
        if(area_row->n_ospf_vlinks > 0)
        {
            ...
            for (j = 0; j < area_row->n_ospf_vlinks; j++)
            {
                ...
                {
                    if (!strcmp (vlink_row->ospf_auth_type,OVSREC_OSPF_VLINK_OSPF_AUTH_TYPE_TEXT))
                        vtysh_ovsdb_cli_print(p_msg, "%4sarea %s virtual-link %s authentication", "",
                          area_str, vlink_str);
                    else if (!strcmp (vlink_row->ospf_auth_type,OVSREC_OSPF_VLINK_OSPF_AUTH_TYPE_MD5))
                        vtysh_ovsdb_cli_print(p_msg, "%4sarea %s virtual-link %s authentication "\
                        "message-digest", "", area_str, vlink_str);
                    else if (!strcmp (vlink_row->ospf_auth_type,OVSREC_OSPF_VLINK_OSPF_AUTH_TYPE_NULL))
                        vtysh_ovsdb_cli_print(p_msg, "%4sarea %s virtual-link %s authentication "\
                        "null", "", area_str, vlink_str);
                }
                intervals = smap_get_int(&(vlink_row->other_config),OSPF_KEY_HELLO_INTERVAL,OSPF_HELLO_INTERVAL_DEFAULT);
                if (intervals != OSPF_HELLO_INTERVAL_DEFAULT)
                {
                    vtysh_ovsdb_cli_print(p_msg, "%4sarea %s virtual-link %s hello-interval %ld", "",
                          area_str, vlink_str, intervals);
                }
                intervals = smap_get_int(&(vlink_row->other_config),OSPF_KEY_DEAD_INTERVAL,OSPF_DEAD_INTERVAL_DEFAULT);
                if (intervals != OSPF_DEAD_INTERVAL_DEFAULT)
                {
                    vtysh_ovsdb_cli_print(p_msg, "%4sarea %s virtual-link %s dead-interval %ld", "",
                          area_str, vlink_str, intervals);
                }
                intervals = smap_get_int(&(vlink_row->other_config),OSPF_KEY_RETRANSMIT_INTERVAL,OSPF_RETRANSMIT_INTERVAL_DEFAULT);
                if (intervals != OSPF_RETRANSMIT_INTERVAL_DEFAULT)
                {
                    vtysh_ovsdb_cli_print(p_msg, "%4sarea %s virtual-link %s retransmit-interval %ld", "",
                          area_str, vlink_str, intervals);
                }
                intervals = smap_get_int(&(vlink_row->other_config),OSPF_KEY_TRANSMIT_DELAY,OSPF_TRANSMIT_DELAY_DEFAULT);
                if (intervals != OSPF_TRANSMIT_DELAY_DEFAULT)
                {
                    vtysh_ovsdb_cli_print(p_msg, "%4sarea %s virtual-link %s transmit-delay %ld", "",
                          area_str, vlink_str, intervals);
                }
                if (vlink_row->ospf_auth_text_key)
                    vtysh_ovsdb_cli_print(p_msg, "%4sarea %s virtual-link %s authentication-key %s", "",
