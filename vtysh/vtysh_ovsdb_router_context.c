...
vtysh_router_context_ospf_clientcallback(void *p_private)
{
    ...
    OVSREC_VRF_FOR_EACH(ovs_vrf, p_msg->idl)
    {
        for (j = 0; j < ovs_vrf->n_ospf_routers; j++)
        {
            ...
            /*Default metric*/
            metric = smap_get_int(&ospf_router_row->other_config, OSPF_KEY_ROUTER_DEFAULT_METRIC, OSPF_DEFAULT_METRIC_DEFAULT);
            if ((metric > 0) && (metric != atoi(OSPF_DEFAULT_METRIC_DEFAULT)))
            {
                 vtysh_ovsdb_cli_print(p_msg, "%4s%s %d", "", "default-metric", metric);
            }
            ...
            {
              vtysh_ovsdb_cli_print(p_msg, "%4s%s %d", "", "timers lsa-group-pacing", timers);
            }
