        ...
        "Virtual interface number\n")
{
    ...
    if((loopback_number < MIN_LOOP_INTF_RANGE) ||
         (loopback_number > MAX_LOOP_INTF_RANGE)){
...
loopback_if_del_ipv6 (const char *if_name, const char *ipv6)
{
    ...
    if (!port_row)
    {
    vty_out (vty,"%s Interface does not have any port configuration",
             if_name);
...
DEFUN (cli_intf_show_intferface_loopback,
        ...
        "Select a loopback interface\n")
{
    return cli_intf_show_intferface_loopback_if(self, vty, vty_flags, 0, argv);
}
