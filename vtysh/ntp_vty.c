...
const int
ntp_server_replace_parameters(struct ovsrec_ntp_associations *ntp_assoc_row, ntp_cli_ntp_server_params_t *ntp_server_params, bool server_exists)
{
...
const int
ntp_auth_key_replace_parameters(struct ovsrec_ntp_keys *ntp_auth_key_row, ntp_cli_ntp_auth_key_params_t *pntp_auth_key_params, bool key_exists)
{
    if (ntp_auth_key_row) {
        if (!key_exists) {
...
//        vty_out(vty, "%5s", ((buf != NULL) ? buf : ""));
        ...
        vty_out(vty, "  %20s", ntp_assoc_row->name);
...
vtysh_ovsdb_show_ntp_status()
{
    ...
    vty_out(vty, "NTP has been %s", ((buf != NULL) ? buf : "enabled"));
}
