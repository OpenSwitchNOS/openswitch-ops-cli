    ...
    if (NULL == ntp_assoc_row)
    {
        ...
        else
        {
            ...
            if (NULL == ntp_assoc_row)
            {
                ...
                ERRONEOUS_DB_TXN(ntp_auth_key_txn, "Could not insert a row into the NTP Assoc Table");
            }
            else
            {
...
vtysh_ovsdb_ntp_auth_enable_set(ntp_cli_ntp_auth_enable_params_t *pntp_auth_enable_params)
{
    ...
    if (NULL == ovs_system)
    {
         ...
         ERRONEOUS_DB_TXN(ntp_auth_key_txn, "Could not access the System Table");
    }

    if (ntp_auth_enable_params.no_form)
    {
        smap_replace((struct smap *)&ovs_system->ntp_config, SYSTEM_NTP_CONFIG_AUTHENTICATION_ENABLE, NTP_FALSE);
    }
    else
    {
        smap_replace((struct smap *)&ovs_system->ntp_config, SYSTEM_NTP_CONFIG_AUTHENTICATION_ENABLE, NTP_TRUE);
    }
      ...
      )
{
    ...
    /* Finally call the handler */
    ret_code = vtysh_ovsdb_ntp_auth_enable_set(&ntp_server_params);
