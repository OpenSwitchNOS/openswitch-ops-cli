...
static int show_source_interface_selection(source_interface_arguments type)
{
    ...
    switch(type)
    {
        case ALL_PROTOCOL:
        {
            ...
            */
            buff = (char *)smap_get(&row->other_config, "source_interface_tftp");
            if (buff == NULL) {
                buff = (char *)smap_get(&row->other_config, "source_interface_all");
                if(buff==NULL) {
...
| Responsibility : To set the source interface to tftp server in ovsdb system table other config column
| Parameters : const char* soource : pointer to char
|              add : bool type
| Return : CMD_SUCCESS for success , CMD_OVSDB_FAILURE for failure
-----------------------------------------------------------------------------*/
static int source_interface_selection(const char *source, source_interface_arguments type, bool add)
{
    ...
    switch(type)
    {
        ...
        {
            /*
            * To set the source interface to all the specified server protocols
