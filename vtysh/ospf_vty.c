...
char *
ospf_nbr_options_print (int64_t options)
{
...
ospf_router_tbl_default(const struct ovsrec_ospf_router *ospf_router_row)
{
    ...
    int64_t stub_values[OSPF_ROUTER_STUB_ROUTER_CONFIG_MAX];
    ...
    ovsrec_ospf_router_set_ospf_restart(ospf_router_row, &smap);
    ...
    stub_keys[OSPF_ROUTER_STUB_ROUTER_ADV_ADMIN]    =
                                                OSPF_KEY_ROUTER_STUB_ADMIN;
    stub_values[OSPF_ROUTER_STUB_ROUTER_ADV_ADMIN]  =
                                                OSPF_ROUTER_STUB_ADMIN_DEFAULT;
    stub_keys[OSPF_ROUTER_STUB_ROUTER_ADV_EXT_LSA]    =
                                                OSPF_KEY_ROUTER_STUB_EXT_LSA;
    stub_values[OSPF_ROUTER_STUB_ROUTER_ADV_EXT_LSA]  =
                                              OSPF_ROUTER_STUB_EXT_LSA_DEFAULT;
    stub_keys[OSPF_ROUTER_STUB_ROUTER_ADV_INCLUDE_STUB]    =
                                                OSPF_KEY_ROUTER_STUB_INCLUDE;
    stub_values[OSPF_ROUTER_STUB_ROUTER_ADV_INCLUDE_STUB] =
                                              OSPF_ROUTER_STUB_INCLUDE_DEFAULT;
    stub_keys[OSPF_ROUTER_STUB_ROUTER_ADV_STARTUP]      =
                                            OSPF_KEY_ROUTER_STUB_STARTUP;
    stub_values[OSPF_ROUTER_STUB_ROUTER_ADV_STARTUP]    =
                                            OSPF_ROUTER_STUB_STARTUP_DEFAULT;
    stub_keys[OSPF_ROUTER_STUB_ROUTER_ADV_STARTUP_WAITBGP]   =
                                            OSPF_KEY_ROUTER_STUB_STARTUP_WAIT;
    stub_values[OSPF_ROUTER_STUB_ROUTER_ADV_STARTUP_WAITBGP] =
                                         OSPF_ROUTER_STUB_STARTUP_WAIT_DEFAULT;
    stub_keys[OSPF_ROUTER_STUB_ROUTER_ADV_SUMMARY_LSA]    =
                                         OSPF_KEY_ROUTER_STUB_SUMMARY_LSA;
    stub_values[OSPF_ROUTER_STUB_ROUTER_ADV_SUMMARY_LSA] =
                                        OSPF_ROUTER_STUB_SUMMARY_LSA_DEFAULT;
    ovsrec_ospf_router_set_stub_router_adv(ospf_router_row, stub_keys,
...
/*Funtion to get the area type from area table. */
bool
ospf_get_area_type(const struct ovsrec_ospf_area* ospf_area_row,
                             const char *key)
{
    int i = 0;

    if (!ospf_area_row || !key)
        return false;

    for (i = 0; i < ospf_area_row->n_area_type; i++)
    {
        if (!strcmp(ospf_area_row->key_area_type[i], key))
            return ospf_area_row->value_area_type[i];
    }
                             ...
                             const struct ovsrec_ospf_area *ospf_area_row)
{
    ...
    if(ospf_area_row->ospf_auth_type)
    {
        ...
        strcmp(ospf_area_row->ospf_auth_type, OVSREC_OSPF_AREA_OSPF_AUTH_TYPE_MD5)== 0 ? "message digest": \
        strcmp(ospf_area_row->ospf_auth_type, OVSREC_OSPF_AREA_OSPF_AUTH_TYPE_TEXT) == 0 ? "text" : "no",
        VTY_NEWLINE);
    }
    else
    {
    ...
    /* Router LSA */
    val = smap_get(&ospf_area_row->status, OSPF_KEY_AREA_ROUTER_CHKSUM);
    if (val)
    {
        vty_out(vty, "    Number of router LSA %ld. Checksum Sum 0x%08x %s",
                ospf_area_row->n_router_lsas, atoi(val), VTY_NEWLINE);
    }
    else
    {
    ...
    /* Network LSA */
    val = smap_get(&ospf_area_row->status, OSPF_KEY_AREA_NETWORK_CHKSUM);
    if (val)
    {
        vty_out(vty, "    Number of network LSA %d. Checksum Sum 0x%08x %s",
                ospf_area_row->n_network_lsas, atoi(val), VTY_NEWLINE);
    }
    else
    {
    ...
    /* ABR Summary LSA */
    val = smap_get(&ospf_area_row->status, OSPF_KEY_AREA_ABR_SUMMARY_CHKSUM);
    if (val)
    {
    ...
    /* ASBR Summary LSA */
    val = smap_get(&ospf_area_row->status, OSPF_KEY_AREA_ASBR_SUMMARY_CHKSUM);
    if (val)
    {
    ...
    /* NSSA LSA */
    val = smap_get(&ospf_area_row->status, OSPF_KEY_AREA_NSSA_CHKSUM);
    if (val)
    {
    ...
    /* Opaque link LSA */
    val = smap_get(&ospf_area_row->status, OSPF_KEY_AREA_OPAQUE_LINK_CHKSUM);
    if (val)
    {
    ...
    /* Opaque area LSA */
    val = smap_get(&ospf_area_row->status, OSPF_KEY_AREA_OPAQUE_AREA_CHKSUM);
    if (val)
    {
...
ospf_ip_router_show(int br_flag)
{
    ...
    else
    {
        ...
        val = smap_get(&ospf_router_row->status, OSPF_KEY_RFC1583_COMPATIBLE);
        if (val)
        {
        ...
        /* Opaque capability */
        val = smap_get(&ospf_router_row->status, OSPF_KEY_CAPABILITY_OPAQUE);
        if (val)
        {
        ...
        /* Stub router support */
        val = smap_get(&ospf_router_row->status, OSPF_KEY_ENABLE_STUB_ROUTER_SUPPORT);
        if (val)
        {
        ...
        /* Stub Startup  time */
        if(ospf_router_row->key_stub_router_adv[OSPF_ROUTER_STUB_ROUTER_ADV_STARTUP])
        {
           vty_out(vty, "      Enabled for %ds after start-up%s",
           ospf_router_row->value_stub_router_adv[OSPF_ROUTER_STUB_ROUTER_ADV_STARTUP],
                VTY_NEWLINE);
            }
        }
        else
        ...
        if(ospf_router_row->key_spf_calculation[OSPF_SPF_DELAY])
        {
            vty_out(vty, "  Initial SPF scheduling delay %d millisec(s)%s",
                 ospf_router_row->value_spf_calculation[OSPF_SPF_DELAY],
                 VTY_NEWLINE);
        }
        else
        {
        ...
        /* Minimum hold time */
        if(ospf_router_row->key_spf_calculation[OSPF_SPF_HOLD_TIME])
        {
            vty_out(vty, "  Minimum hold time between consecutive SPFs %d millisec(s)%s",
                 ospf_router_row->value_spf_calculation[OSPF_SPF_HOLD_TIME],
                 VTY_NEWLINE);
        }
        else
        {
        ...
        /* Maximum hold time */
        if(ospf_router_row->key_spf_calculation[OSPF_SPF_MAX_WAIT])
        {
            vty_out(vty, "  Maximum hold time between consecutive SPFs %d millisec(s)%s",
                 ospf_router_row->value_spf_calculation[OSPF_SPF_MAX_WAIT],
                 VTY_NEWLINE);
        }
        else
        {
        ...
        /* Adjacency logging */
        val = smap_get(&ospf_router_row->other_config, OSPF_KEY_LOG_ADJACENCY_CHGS);
        if (val)
        {
                        ...
                        const struct ovsrec_ospf_interface *ospf_interface_row)
{
    ...
    if (ospf_get_area_type(ospf_area_row, OSPF_KEY_AREA_TYPE_NSSA))
    {
        vty_out(vty, " [NSSA]");
    }
    else if(ospf_get_area_type(ospf_area_row, OSPF_KEY_AREA_TYPE_STUB))
    {
        vty_out(vty, " [Stub]");
    }
    vty_out(vty, "%s", VTY_NEWLINE);
                                ...
                                bool all_flag)
{
    ...
    if (ospf_nbr_row->dr)
    {
        snprintf(state_str, OSPF_SHOW_STR_LEN, "%s/%s",
                 ospf_nbr_row->nfsm_state, "DR");
    }
    else if (ospf_nbr_row->bdr)
    {
        snprintf(state_str, OSPF_SHOW_STR_LEN, "%s/%s",
                 ospf_nbr_row->nfsm_state, "BDR");
    }
    else
    {
        snprintf(state_str, OSPF_SHOW_STR_LEN, "%s/%s",
                 ospf_nbr_row->nfsm_state, "DROther");
    }
                                ...
                                bool all_flag)
{
    ...
    if (area_row)
    {
        if (ospf_get_area_type(area_row, OSPF_KEY_AREA_TYPE_NSSA))
        {
            vty_out(vty, " [NSSA] ");
        }
        else if(ospf_get_area_type(area_row, OSPF_KEY_AREA_TYPE_STUB))
        {
            vty_out(vty, " [Stub] ");
        }
    }
    ...
    if(ospf_nbr_row->nbr_priority)
    {
        vty_out (vty, "    Neighbor priority is %d, "   \
                 "State is %s, %d state changes%s",
                 *ospf_nbr_row->nbr_priority, ospf_nbr_row->nfsm_state,
                 ospf_get_statistics_from_neighbor(ospf_nbr_row,
    ...
    if (ospf_nbr_row->nbr_options)
    {
        vty_out (vty, "    Options %d  (%s)%s", *ospf_nbr_row->nbr_options,
                ospf_nbr_options_print(*ospf_nbr_row->nbr_options), VTY_NEWLINE);
    }
    else
    {
