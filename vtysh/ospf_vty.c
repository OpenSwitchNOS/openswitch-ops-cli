                                                  ...
                                                  int area_id)
{
    ...
    if (!ifname)
    {
        ABORT_DB_TXN(ospf_router_txn, "Interface not present");
        return CMD_WARNING;
    }
    ...
    for (i = 0; i < ospf_router_row->n_areas; i++)
    {
        ...
        if (area != area_id)
        {
            ...
            for (j = 0; j < area_row->n_ospf_interfaces; j++)
            {
                ...
                if (strcmp(interface_name, ifname) == 0)
                {
                        ABORT_DB_TXN(ospf_router_txn, "Remove the existing"
    ...
    if (NULL == port_row)
    {
        ABORT_DB_TXN(ospf_router_txn, "No port to add ospf "
    ...
    if (is_present == false)
    {
        ABORT_DB_TXN(ospf_router_txn, "Interface not present in the port.");
        return CMD_OVSDB_FAILURE;
    }
    ...
    {
        ABORT_DB_TXN(ospf_router_txn, "OSPF configuration not allowed "
                        "in LACP port.");
        return CMD_WARNING;
    }
                                                  ...
                                                  int area_id)
{
    ...
    if (is_present == false)
    {
        ABORT_DB_TXN(ospf_router_txn, "Area id was not configured.");
        return CMD_WARNING;
    }
    ...
    if (ospf_if_config_row == NULL)
    {
        ...
        ABORT_DB_TXN(ospf_router_txn, "ospf interface configurations missing.");
        return CMD_OVSDB_FAILURE;
    }
    ...
    if (ospf_router_row == NULL)
    {
        ABORT_DB_TXN(ospf_router_txn, "ospf router configurations missing.");
        return CMD_OVSDB_FAILURE;
    }
    ...
    if (area_row == NULL)
    {
        ABORT_DB_TXN(ospf_router_txn, "ospf area configurations missing.");
        return CMD_OVSDB_FAILURE;
    }
    ...
    ovsrec_ospf_interface_set_name(ospf_interface_row, "");
