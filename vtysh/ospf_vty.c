...
ospf_lsa_database_detail_show(int type, char* link_id)
{
   ...
   else
   {
    ...
    for(i = 0; i < router_row->n_areas; i++)
    {
       ...
       area_row = router_row->value_areas[i];
       if (area_row->area_type && (type != OSPF_LSA_LSA_TYPE_TYPE11_OPAQUE_AS_LSA &&
                    type != OSPF_LSA_LSA_TYPE_TYPE5_AS_EXTERNAL_LSA))
       {
           if(!strcmp(area_row->area_type,
              OVSREC_OSPF_AREA_AREA_TYPE_NSSA) ||
       ...
       if(type != OSPF_LSA_LSA_TYPE_TYPE11_OPAQUE_AS_LSA &&
                    type != OSPF_LSA_LSA_TYPE_TYPE5_AS_EXTERNAL_LSA)
