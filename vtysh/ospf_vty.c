...
ospf_router_tbl_default(const struct ovsrec_ospf_router *ospf_router_row)
{
    ...
    smap_init(&smap);
    smap_add(&smap, OSPF_KEY_ROUTER_STUB_ADMIN, OSPF_ROUTER_STUB_ADMIN_DEFAULT);
                                ...
                                bool all_flag)
{
    ...
    if (val && ( strcmp(val, OSPF_DEFAULT_STR) != 0))
    {
        ...
        /* SUGANYA - TBD 
            1.  How to compare for self router id in dynamic case
            2.  What is the byte order stored for nbr router id?
            */
