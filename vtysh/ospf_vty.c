                        ...
                        const struct ovsrec_ospf_interface *ospf_interface_row)
{
    ...
    for (j = 0; j < ospf_interface_row->n_neighbors; j++)
    {
        ...
        if ((!is_dr_present) && (!is_bdr_present) &&
            (ospf_nbr_row->n_nbr_router_id > 0) &&
            (ntohl(*ospf_nbr_row->nbr_router_id) == ntohl(router_id)))
        {
            ...
            /* BDR */
            if((ospf_nbr_row->bdr) && (ospf_nbr_row->nbr_if_addr))
            {
                memset(show_str,'\0', OSPF_SHOW_STR_LEN);
            ...
            /* Don't count this self router entry */
            continue;
        }
                                ...
                                bool all_flag)
{
    ...
    if ((ospf_nbr_row->dr) && (ospf_nbr_row->nbr_router_id) &&
       (*ospf_nbr_row->dr == *ospf_nbr_row->nbr_router_id))
    {
        snprintf(state_str, OSPF_SHOW_STR_LEN, "%s/%s",
                 ospf_nbr_row->nfsm_state, "DR");
    }
    else if ((ospf_nbr_row->bdr) && (ospf_nbr_row->nbr_router_id) &&
       (*ospf_nbr_row->bdr == *ospf_nbr_row->nbr_router_id))
    {
        snprintf(state_str, OSPF_SHOW_STR_LEN, "%s/%s",
                                     ...
                                     bool all_flag)
{
    ...
    else if (nbr_id != 0)
    {
        ...
        OVSREC_OSPF_NEIGHBOR_FOR_EACH(ospf_nbr_row, idl)
        {
            if (nbr_id == ntohl(*ospf_nbr_row->nbr_router_id))
                ospf_neighbor_one_row_print(ospf_nbr_row, all_flag);
