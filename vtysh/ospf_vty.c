  ...
  if(lsa_row->ls_birth_time)
   ...
   if(lsa_row->ls_seq_num)
       vty_out (vty, "  LS Seq Number: %08lx%s",
                       lsa_row->ls_seq_num,VTY_NEWLINE);
   else
   {
       vty_out (vty, "   LS Seq Number: %s%s", OSPF_DEFAULT_STR, VTY_NEWLINE);
   }
   if (lsa_row->n_chksum > 0)
       vty_out (vty, "  Checksum: 0x%04x%s",
                        lsa_row->chksum,VTY_NEWLINE);
   else
        vty_out(vty, "  Checksum: 0x%04x%s",
   ...
   else
   {
	...
	for(i = 0; i < router_row->n_areas; i++)
	{
	   ...
	   switch(type)
	   {
	       ...
	       {
	           vty_out(vty,"        AS External Link States (Area %s",
                                                                      area_str);
			   break;
		   }
	       ...
	       {
	           vty_out(vty,"        Opaque-AS Link States (Area %s",
                                                                     area_str);
			   break;
		   }
		   default:
...
ospf_ip_database_show()
{
    ...
    /* See if it already exists. */
     ...
     if (val)
          ...
          vty_out(vty, "%s       OSPF Router with ID (%s)%s",
                                 VTY_NEWLINE,OSPF_DEFAULT_STR, VTY_NEWLINE);
