...
static int
ospf_router_default_metric_cmd_exec (bool n_flag, const char * metric)
{
    const struct ovsrec_ospf_router *ospf_router_row = NULL;
    const struct ovsrec_vrf *vrf_row = NULL;
    struct ovsdb_idl_txn *ospf_router_txn=NULL;
    ...
    if (n_flag == false){
      value_metric = smap_get(&smap, OSPF_KEY_ROUTER_DEFAULT_METRIC);
      if (atoi(value_metric) != atoi(metric)){
          OSPF_ABORT_DB_TXN(ospf_router_txn,
                           "Value was not configured earlier.");
          return CMD_SUCCESS;
      }
      smap_replace(&smap,  OSPF_KEY_ROUTER_DEFAULT_METRIC, CONST_CAST(char*, OSPF_DEFAULT_METRIC_DEFAULT));
    }else
...
int
ospf_update_timers_for_lsa_group_pacing(bool n_flag,
                                        const struct ovsrec_ospf_router *
                                            ospf_router_row,
                                        int duration)
{
    int64_t *value_list;
    ...
    key_list =
      xmalloc(OSPF_TIMER_KEY_MAX_LENGTH * (ospf_router_row->n_lsa_timers + 1));
    value_list =
...
static int
ospf_router_timers_lsa_group_pacing_set(bool n_flag, int duration)
{
    const struct ovsrec_ospf_router *ospf_router_row = NULL;
    const struct ovsrec_vrf *vrf_row = NULL;
    struct ovsdb_idl_txn *ospf_router_txn=NULL;
    ...
    if (ospf_update_timers_for_lsa_group_pacing(n_flag, ospf_router_row, duration) != CMD_SUCCESS)
    {
...
static int
ospf_router_compat_rfc1583_cmd_exe(bool no_flag)
{
    const struct ovsrec_ospf_router *ospf_router_row = NULL;
    const struct ovsrec_vrf *vrf_row = NULL;
    struct ovsdb_idl_txn *ospf_router_txn=NULL;
       ...
       "Default metric\n")
{
    if (argc)
      ...
      return ospf_router_default_metric_cmd_exec(true, CONST_CAST(char*, OSPF_DEFAULT_METRIC_DEFAULT));
}
      ...
      "Set timer for OSPF LSA group-pacing\n")
{
      if (argc)
        ...
        return ospf_router_timers_lsa_group_pacing_set(true, OSPF_LSA_GROUP_PACING_DEFAULT);
}
