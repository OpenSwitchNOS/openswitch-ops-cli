...
vrf_add(const char *vrf_name)
{
  ...
  if (!ovs_row)
    {
      vty_out(vty, "Could not fetch System data.%s",
          VTY_NEWLINE);
      VLOG_DBG(
          "%s System table did not have any rows. Ideally it"
...
vrf_del_port(const char *if_name, const char *vrf_name)
{
  ...
  if (!port_row) {
    vty_out(vty, "Interface %s not attached to given VRF.%s",
        if_name, VTY_NEWLINE);
...
vrf_del_ip(const char *if_name, const char *ip4, bool secondary)
{
  ...
  if (!secondary)
    ...
    {
      vty_out(vty, "No IP address configured on interface %s.%s",
          if_name, VTY_NEWLINE);
    ...
    {
      vty_out(vty,
          "No secondary IP address configured on"
    ...
    {
      vty_out(vty, "IP address %s not found.%s",
          ip4, VTY_NEWLINE);
      VLOG_DBG("%s IP address \"%s\" not configured on interface"
          " \"%s\".", __func__, ip4, if_name);
      cli_do_config_abort(status_txn);
    ...
    secondary_ip4_addresses = xmalloc(IP_ADDRESS_LENGTH *
                      (port_row->n_ip4_address_secondary - 1));
...
vrf_del_ipv6(const char *if_name, const char *ipv6, bool secondary)
{
  ...
  if (!secondary)
    ...
    if (strcmp(port_row->ip6_address, ipv6) != 0)
      {
    vty_out(vty, "IPv6 address %s not found.%s", ipv6,
        VTY_NEWLINE);
    VLOG_DBG(
    ...
    {
      vty_out(vty,
          "No secondary IPv6 address configured on interface"
          " %s.%s", if_name, VTY_NEWLINE);
    ...
    secondary_ipv6_addresses = xmalloc(IPV6_ADDRESS_LENGTH *
                       (port_row->n_ip6_address_secondary - 1));
