...
class ShowRunningConfigTests( HalonTest ):
  ...
  def enablelldpTest(self):
    ...
    for line in lines:
        if "feature lldp" in line:
            enable_lldp = True
    assert enable_lldp == True, 'Test to verify show running-config for feature lldp - FAILED!'
    return True
  ...
  def setlldpholdtimeTest(self):
    ...
    assert set_lldp_hold_time == True,'Test to verify show running-config for lldp holdtime - FAILED!'
    return True
  ...
  def disablelldptxdirTest(self):
    ...
    assert lldp_txrx_disabled == True,'Test to verify show running-config for lldp transmission - FAILED!'
    return True
  ...
  def setLogrotatePeriodTest(self):
    ...
    lines = out.split('\n')
    for line in lines:
        assert "logrotate period" not in line, 'Default behavior: logrotate period should not be part of running config'
    ...
    lines = out.split('\n')
    for line in lines:
        assert "logrotate period none" not in line, 'Default behavior: logrotate period none should not be part of running config'
    ...
    assert logrotate_period_set == True, 'Test to verify show running-config for logrotate period'
    return True
  ...
  def setLogrotateMaxsizeTest(self):
    ...
    lines = out.split('\n')
    for line in lines:
        assert "logrotate maxsize" not in line, 'Default behavior: logrotate maxsize should not be part of running config'
    ...
    lines = out.split('\n')
    for line in lines:
        assert "logrotate maxsize 10" not in line, 'Default behavior: logrotate maxsize 10 should not be part of running config'
    ...
    assert logrotate_max_size_set == True, 'Test to verify show running-config for logrotate maxsize - FAILED!'
    return True
  ...
  def setLogrotateTargetTest(self):
    ...
    lines = out.split('\n')
    for line in lines:
        assert "logrotate target" not in line, 'Default behavior: logrotate target should not be part of running config'
    ...
    lines = out.split('\n')
    for line in lines:
        assert "logrotate target local" not in line, 'Default behavior: logrotate target local should not be part of running config'
    ...
    assert logrotate_target_set == True, 'Test to verify show running-config for logrotate target - FAILED!'
    return True
