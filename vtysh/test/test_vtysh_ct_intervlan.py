...
second_interface = "2"
third_interface = "3"
error_str = "Error: Invalid vlan ID. Enter valid Vlan ID in the range of <1 to 4094> AND NOT part of internal VLAN"
max_vlan = "4095"
min_vlan = "0"
...
class intervlanCLITest( HalonTest ):
    ...
    def test_intervlan_input_test(self):
        ...
        assert error_str in ret, 'Adding Vlan id = %s validation failed' %max_vlan
        intf_cmd = 'interface vlan' + max_vlan
        ret = s1.cmdCLI(intf_cmd)
        assert 'Unknown command.' in ret, 'Adding Vlan id = %s validation failed' %max_vlan
        intf_cmd = 'interface vlan ' + min_vlan
        ret = s1.cmdCLI(intf_cmd)
        assert error_str in ret, 'Adding Vlan id = 0 validation failed'
        intf_cmd = 'interface vlan' + min_vlan
        ret = s1.cmdCLI(min_vlan)
        assert 'Unknown command.' in ret, 'Adding Vlan id = 0 validation failed'
        ...
        assert 'Unknown command.' in ret, 'Vlan id = 2abc validation failed'
        ...
        assert 'Unknown command.' in ret, 'Vlan id = abc2abc validation failed'
        ...
        # Deleting interface vlan outsode range <1-4094>
        intf_cmd = 'no interface vlan ' + max_vlan
        ret = s1.cmdCLI(intf_cmd)
        assert error_str in ret, 'Deleting vlan id = %s validation failed' %max_vlan
        intf_cmd = 'no interface vlan' + max_vlan
        ret = s1.cmdCLI(intf_cmd)
        assert 'Unknown command.' in ret, 'Deleting vlan id = %s validation failed' %max_vlan
        intf_cmd = 'no interface vlan ' + min_vlan
        ret = s1.cmdCLI(intf_cmd)
        assert error_str in ret, 'Deleting vlan id = %s validation failed' %min_vlan
        intf_cmd = 'no interface vlan' + min_vlan
        ret = s1.cmdCLI(intf_cmd)
        assert 'Unknown command.' in ret, 'Deleting vlan id = %s validation failed' %min_vlan
    ...
    def test_vlan_interface_add_delete(self):
        ...
        # Adding vlan interface and verify not equal to internal vlan
        s1.cmdCLI("configure terminal")
        s1.cmdCLI("vlan internal range 1 4094 ascending")
        ...
        intf_cmd = "interface vlan " + first_interface
        ret = s1.cmdCLI(intf_cmd)
        assert error_str in ret, 'Failed to verify internal vlan'
        info('### Internal vlan validation passed ###\n')
    ...
    def test_show_running_config(self):
        ...
        assert 'ip address 10.1.1.1/8' in output[intf2_output+2] and \
               'ip address 10.1.1.3/8 secondary' in output[intf2_output+3] and \
               'ipv6 address 2002::1/120' in output[intf2_output+4] and \
               'ipv6 address 2002::2/120 secondary' in output[intf2_output+5], \
                'Show running config for ' + intf2 + ' failed'
        info('### Show running config for ' + intf2 + ' passed ###\n')
        assert 'no shutdown' in output[intf3_output+1], 'Show running ' \
