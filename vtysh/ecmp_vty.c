...
static int ecmp_config_set_status(const char *status, const char * feild)
{
    ...
    VLOG_DBG("%s status = %s", __func__, status);
    if ( (strcmp("true",status) == 0) || (strcmp("false", status) == 0)) {
        VLOG_DBG("%s Replace status = %s", __func__, status);
        smap_replace(&ovs_row->ecmp_config, feild, status);
    } else {
        VLOG_ERR(OVSDB_ROW_FETCH_ERROR);
        cli_do_config_abort(status_txn);
        return CMD_OVSDB_FAILURE;
    ...
    VLOG_DBG("%s Set the ecmp config in OVSDB", __func__);
       ...
       ECMP_CONFIG_DISABLE_STR)
{
    ...
    return ecmp_config_set_status(ECMP_CONFIG_DISABLE, ECMP_CONFIG_STATUS);
}
       ...
       "Disable load balancing by source IP\n")
{
    ...
    return ecmp_config_set_status(ECMP_CONFIG_DISABLE, ECMP_CONFIG_HASH_SRC_IP);
}
       ...
       "Disable load balancing by destination IP\n")
{
    ...
    return ecmp_config_set_status(ECMP_CONFIG_DISABLE, ECMP_CONFIG_HASH_DST_IP);
}
       ...
       "Disable load balancing by destination port\n")
{
    ...
    return ecmp_config_set_status(ECMP_CONFIG_DISABLE, ECMP_CONFIG_HASH_DST_PORT);
}
       ...
       "Disable load balancing by source port\n")
{
    ...
    return ecmp_config_set_status(ECMP_CONFIG_DISABLE, ECMP_CONFIG_HASH_SRC_PORT);
}
       ...
       ECMP_CONFIG_DISABLE_STR)
{
    ...
    return ecmp_config_set_status(ECMP_CONFIG_ENABLE_DEFAULT, ECMP_CONFIG_STATUS);
}
       ...
       "Disable load balancing by source IP\n")
{
    ...
    return ecmp_config_set_status(ECMP_CONFIG_ENABLE_DEFAULT, ECMP_CONFIG_HASH_SRC_IP);
}
...
{
    VLOG_DBG("Add hash option Destination IP");
    return ecmp_config_set_status(ECMP_CONFIG_ENABLE_DEFAULT, ECMP_CONFIG_HASH_DST_IP);
}
...
{
    VLOG_DBG("Add hash option Destination Port");
    return ecmp_config_set_status(ECMP_CONFIG_ENABLE_DEFAULT, ECMP_CONFIG_HASH_DST_PORT);
}
...
{
    VLOG_DBG("Add hash option Source Port");
    return ecmp_config_set_status(ECMP_CONFIG_ENABLE_DEFAULT, ECMP_CONFIG_HASH_SRC_PORT);
}
       ...
       "ECMP Configuration\n")
{
    ...
    vty_out(vty, "\nECMP Configuration\n");
    vty_out(vty, "---------------------\n\n");
    vty_out(vty, "ECMP Status        : %s%s", GET_ECMP_CONFIG_STATUS(ovs_row)?"Enabled":"Disabled",VTY_NEWLINE);
    vty_out(vty, "\nECMP Load Balancing by\n");
    vty_out(vty, "------------------------\n");
    vty_out(vty, "Source IP          : %s%s", GET_ECMP_CONFIG_HASH_SRC_IP_STATUS(ovs_row)?"Enabled":"Disabled",VTY_NEWLINE);
    vty_out(vty, "Destination IP     : %s%s", GET_ECMP_CONFIG_HASH_DST_IP_STATUS(ovs_row)?"Enabled":"Disabled",VTY_NEWLINE);
    vty_out(vty, "Source Port        : %s%s", GET_ECMP_CONFIG_HASH_SRC_PORT_STATUS(ovs_row)?"Enabled":"Disabled",VTY_NEWLINE);
    vty_out(vty, "Destination Port   : %s%s\n", GET_ECMP_CONFIG_HASH_DST_PORT_STATUS(ovs_row)?"Enabled":"Disabled",VTY_NEWLINE);
    return CMD_SUCCESS;
}
