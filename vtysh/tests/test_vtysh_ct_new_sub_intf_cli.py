...
# creating asub interface and verifying in port and interface table
    ...
    devIntReturn = device1.DeviceInteract(command="ovs-vsctl get interface 4.2 name")
    ...
    devIntReturn = device1.DeviceInteract(command="ovs-vsctl get port 4.2 name")
    retCode = devIntReturn.get('buffer')
    assert "4.2"in retCode ,"Failed to retrieve ovs-vsctl command"
    LogOutput('info', "### interface 4.2 created successfully ###")
...
# configuring the ip address and verifying ip address in port table
    ...
    devIntReturn = device1.DeviceInteract(command="ovs-vsctl get port 4.3 ip4_address")
    retCode = devIntReturn.get('buffer')
...
# verifing in assigning Invalid Ip address
    ...
    if 'Invalid IP address' in retCode:
        LogOutput('info',"Invalid IP address")
...
#configuring the ipv6 address and verifying in port table
    retStruct = InterfaceIpConfig(deviceObj=device1,
                                  interface="4.4",
                                  addr="10:10::10:10",ipv6flag=True, mask=24, config=True)
    devIntReturn = device1.DeviceInteract(command="ovs-vsctl get port 4.4 ip6_address")
    retCode = devIntReturn.get('buffer')
    assert '10:10::10:10/24' in retCode, \
                    'Test to verify sub-interface configuration clis - FAILED!'

#verifing same vlan for different subinterface
    retStruct = Dot1qEncapsulation(deviceObj=device1,subInterface="4.3", dot1q=True, vlan=100)
    if retStruct.returnCode() != 0:
        LogOutput('error',"Failed to enable interface")
        assert(False)

    retStruct = Dot1qEncapsulation(deviceObj=device1,subInterface="4.8", dot1q=True, vlan=100)
    retCode =retStruct.buffer()
    if 'Encapsulation VLAN is already configured on interface 4.3.' in retCode:
        LogOutput('info',"Dot1Q encapsulation already assigned !.")

    retStruct = Dot1qEncapsulation(deviceObj=device1,subInterface="4.8", dot1q=False, vlan=100)
    retCode =retStruct.buffer()
    if 'Encapsulation VLAN is already configured on interface 4.3.' in retCode:
        LogOutput('info',"Dot1Q encapsulation already assigned !.")
    ...
    if 'Duplicate IP Address.' in retCode:
        LogOutput('info',"Ip already assigned !.")

    retStruct = Dot1qEncapsulation(deviceObj=device1,subInterface="4.8", dot1q=False, vlan=100)
    devIntReturn = device1.DeviceInteract(command="do show running-config")
    retCode =retStruct.buffer()
    if 'encapsulation dot1Q 100' in retCode:
        LogOutput('info',"encapsulation dot1Q still exist-Fail!")
    ...
    retCode = devIntReturn.get('buffer')
    if "Invalid input"in retCode :
        LogOutput('info', "Canot creat the interface ")

    devIntReturn = device1.DeviceInteract(command="no int 12.2")
    retCode = devIntReturn.get('buffer')
    if "Interface does not exist"in retCode :
        LogOutput('info', "no interface exist failed!")


    devIntReturn = device1.DeviceInteract(command="int 4.9")
    retCode = devIntReturn.get('buffer')
    print retCode
    if 'Parent interface is not L3' in retCode :
        LogOutput('info',"canot create subinterface  in L2 interface")

    devIntReturn = device1.DeviceInteract(command=  "no int 4.2")

    devIntReturn = device1.DeviceInteract(command="int 100.1025")
    retCode = devIntReturn.get('buffer')
    if "Parent interface does not exis"in retCode :
        LogOutput('info', "parent interface is not present!.")
    ...
    print retCode
    if "no row \"4.2\" in table Interface" in retCode:
       LogOutput('info', "### interface 4.2 deleted ! ###")
