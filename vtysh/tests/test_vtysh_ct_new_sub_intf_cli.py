...
# verifing same vlan for different subinterface
    ...
    devIntReturn = device1.DeviceInteract(command="int 1-1")
    retCode = devIntReturn.get('buffer')
    assert "Invalid input"in retCode, \
           'Cannot creat the interface'

    devIntReturn = device1.DeviceInteract(command="int 11&11")
    retCode = devIntReturn.get('buffer')
    assert "Invalid input"in retCode, \
           'Cannot creat the interface'

    devIntReturn = device1.DeviceInteract(command="int 1 2")
    retCode = devIntReturn.get('buffer')
    assert "Invalid input"in retCode, \
           'Cannot creat the interface'
