...
def ProxyArpOnNoRoutingTest(dut01):
    ...
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Failed to enable no routing"
    "on L3 port"
...
def ProxyArpOnL3VlanInterfaceTest(dut01):
    ...
    cmdOut = devIntReturn.get('buffer')
    assert 'ip proxy-arp' in cmdOut, "Failed to validate the"
    "'ip proxy-arp' in show running-config output"
...
def ProxyArpOnSplitChildInterfaceTest(dut01):
    ...
    'parent interface has not been split.' in cmdOut, "Verify proxy ARP "
    "configuration on a child interface of non-split parent interface fails"
...
def ProxyArpOnParentInterfaceTest(dut01):
    ...
    'Operation not allowed.' in cmdOut, "Verify proxy ARP configuration "
    "fails with an error message on a split parent interface"
    return True
