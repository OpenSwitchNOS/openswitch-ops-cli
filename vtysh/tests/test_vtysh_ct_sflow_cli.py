...
class sflowConfigTest(OpsVsiTest):
    ...
    def setupNet(self):
        ...
        static_topo = myTopo(hsts=0, sws=1, hopts=host_opts, sopts=switch_opts)
    ...
    def test_sflow_global_status(self):
        ...
        info("### Verifiying row name as 'global' for the sflow row "
                "created ###\n")
        cmd = "ovsdb-client monitor sFlow"
    ...
    def test_sflow_show_running(self):
        ...
        assert 'sflow collector 255.255.255.255\\6343\\vrf_default' in \
                out[word_index+1] and \
                'FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF\\65535\\vrf_default' \
                in out[word_index+2] and \
