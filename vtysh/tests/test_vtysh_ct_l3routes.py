...
### Verify if nexthop is not assigned locally to an interface as a primary '''
        ...
        ret = s1.cmdCLI('do show ip route')
        assert not '192.168.2.1' in ret, 'Primary ip address check for ' \
                'nexthop failed'
...
### Verify if nexthop is not assigned locally to an interface as a '''
        ...
        ret = s1.cmdCLI('do show ip route')
        assert not '192.168.2.2'in ret, 'Secondary ip address check for' \
                ' nexthop failed'
        info('### Nexthop ip address verification successful against local '
             'secondary address ###\n\n\n')
        s1.cmdCLI('interface 1')
...
### Verify if nexthop is not assigned locally to an interface as a primary '''
        ...
        ret = s1.cmdCLI('do show ipv6 route')
        assert not '2001::1' in ret, 'Primary ipv6 address check for nexthop' \
                ' failed'
        info('### Nexthop ip address verification successful against local '
             'primary address  ###\n')
...
### Verify if nexthop is not assigned locally to an interface as a '''
        ...
        ret = s1.cmdCLI('do show ipv6 route')
        assert not '2000::3' in ret, 'Secondary ipv6 address check for ' \
                'nexthop failed'
