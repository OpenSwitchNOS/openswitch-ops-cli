...
#creating a loopback interface and verifying in interface and porttable
    ...
    retStruct = LoopbackInterfaceEnable(deviceObj=device1,loopback="1",config=True)
    LogOutput('info', "### Verify the Loopback  interface is created with'\
             ' same name for L3 port ###")

    devIntReturn = device1.DeviceInteract(command="ovs-vsctl get interface lo1 name")
    ...
    devIntReturn = device1.DeviceInteract(command="ovs-vsctl get port lo1 name")
    retCode = devIntReturn.get('buffer')
    assert "lo1"in retCode ,"Failed to retrieve ovs-vsctl command"
...
#assigning ipv4 and verifying whether ip assigned
    retStruct = LoopbackInterfaceEnable(deviceObj=device1,
                                loopback="1", addr="192.168.1.5", mask=24,config=True)
    devIntReturn = device1.DeviceInteract(command="ovs-vsctl get port lo1 ip4_address ")
    retCode = devIntReturn.get('buffer')
...
#verifying dupplicate ip
    retStruct = LoopbackInterfaceEnable(deviceObj=device1,
                                loopback="2", addr="192.168.1.5", mask=24,config=True)
    devIntReturn = device1.DeviceInteract(command="ovs-vsctl get port lo1 ip4_address ")
    retCode = devIntReturn.get('buffer')
    print retCode
...
#assigning and verifying ipv6
    retStruct = LoopbackInterfaceEnable(deviceObj=device1,
                                loopback="1", addr="10:10::10:10",ipv6flag=True, mask=24,config=True)
    devIntReturn = device1.DeviceInteract(command="ovs-vsctl get port lo1 ip6_address")
    retCode = devIntReturn.get('buffer')
    assert '10:10::10:10/24' in retCode, \
                    'Test to verify Loopback-interface configuration clis - FAILED!'

#unassigning the ipv4 and ferifying whether it is unassinged
    retStruct = LoopbackInterfaceEnable(deviceObj=device1,
                                loopback="1", addr="192.168.1.5", mask=24,config=False)
    devIntReturn = device1.DeviceInteract(command="ovs-vsctl get interface lo1 ip4_address")
    retCode = devIntReturn.get('buffer').splitlines()
    if len(retCode) == 2:
        retCode = retCode[1]
    if " " in retCode:
       LogOutput('info', "### Ip unassigend for lo1! ###")

#Deleting the loopback interface
    retStruct = LoopbackInterfaceEnable(deviceObj=device1,loopback="1",config=False)
    devIntReturn = device1.DeviceInteract(command="ovs-vsctl get interface lo1 name")
    retCode = devIntReturn.get('buffer').splitlines()
    if len(retCode) == 3:
        retCode = retCode[1]
...
#assigning invalid ip
    retStruct = LoopbackInterfaceEnable(deviceObj=device1,
                                loopback="1", addr="255.255.255.255", mask=24,config=True)
    retCode = devIntReturn.get('buffer')
    if '% Unknown command.' in retCode :
         LogOutput('info',"Canot creat the loopback interface - FAILED!")
    ...
    devIntReturn = device1.DeviceInteract(command="no int 1")
    ...
    if 'Interface does not exist' in retCode:
         LogOutput('info',"trying to delete a non existing lointerface- FAILED!")
