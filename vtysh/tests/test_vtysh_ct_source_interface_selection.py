...
all address 1.1.1.1")
    ...
    assert '1.1.1.1' in cmdOut, "Test to set source-ip address \
to all servers failed"
...
tftp address 1.1.1.3")
    ...
    assert '1.1.1.3' in cmdOut, "Test to set source-ip address \
to tftp server failed"
...
all interface 2")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to set source-ip interface \
to all servers failed"
    ...
    cmdOut = dut01.cmdVtysh(command="show ip source-interface all")
    assert '2' in cmdOut, "Test to set source-ip interface \
to all servers failed"
...
to tftp server failed"
    ...
    cmdOut = dut01.cmdVtysh(command="show ip source-interface tftp")
    assert '3' in cmdOut, "Test to set source-ip interface \
to tftp server failed"
...
class Test_ospf_configuration:
    ...
    def test_setSourceIpInterfaceToAllServersTest(self):
        ...
        if(retValue):
            LogOutput('info', "Set source IP interface \
to all servers - passed")
        else:
            LogOutput('error', "Set source IP interface \
to all servers - failed")
