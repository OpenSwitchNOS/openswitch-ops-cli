...
def setSourceIpAddressToAllServersTest(dut01):
    ...
    devIntReturn = dut01.DeviceInteract(command="ip source-interface all address 1.1.1.1")
    ...
    cmdOut = dut01.cmdVtysh(command="show ip source-interface all")
    assert '1.1.1.1' in cmdOut, "Test to set source-ip address to all servers failed"
...
def setSourceIpAddressToTftpServerTest(dut01):
    ...
    devIntReturn = dut01.DeviceInteract(command="ip source-interface tftp address 1.1.1.3")
    retCode = devIntReturn.get('returnCode')
    ...
    assert '1.1.1.3' in cmdOut, "Test to set source-ip address to tftp server failed"
...
def setSourceIpInterfaceToAllServersTest(dut01):
    ...
    devIntReturn = dut01.DeviceInteract(command="ip source-interface all interface 2")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to set source-ip interface to all servers failed"

    if (exitContext(dut01) is False):
        return False

    cmdOut = dut01.cmdVtysh(command="show ip source-interface all")
    assert '2' in cmdOut, "Test to set source-ip interface to all servers failed"
...
def setSourceIpInterfaceToTftpServerTest(dut01):
    ...
    devIntReturn = dut01.DeviceInteract(command="ip source-interface tftp interface 3")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Test to set source-ip interface to tftp server failed"

    if (exitContext(dut01) is False):
        return False

    cmdOut = dut01.cmdVtysh(command="show ip source-interface tftp")
    assert '3' in cmdOut, "Test to set source-ip interface to tftp server failed"
...
def unsetSourceIpToAllServersTest(dut01):
    ...
    cmdOut = dut01.cmdVtysh(command="show ip source-interface all")
    assert 'N/A' in cmdOut, "Test to unset source-ip to all servers failed"
...
def unsetSourceIpToTftpServerTest(dut01):
    ...
    cmdOut = dut01.cmdVtysh(command="show ip source-interface tftp")
    assert 'N/A' in cmdOut, "Test to unset source-ip to tftp server failed"
...
class Test_ospf_configuration:
    ...
    def test_setSourceIpInterfaceToAllServersTest(self):
        ...
        if(retValue):
            LogOutput('info', "Set source IP interface to all servers - passed")
        else:
            LogOutput('error', "Set source IP interface to all servers - failed")

    def test_setSourceIpInterfaceToTftpServerTest(self):
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        retValue = setSourceIpInterfaceToTftpServerTest(dut01Obj)
        if(retValue):
            LogOutput('info', "Set source IP interface to tftp server - passed")
        else:
            LogOutput('error', "Set source IP interface to tftp server - failed")
