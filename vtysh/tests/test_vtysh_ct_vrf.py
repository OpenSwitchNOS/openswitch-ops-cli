...
class vrfCLITest( HalonTest ):
    ...
    def test_interface(self):
        ...
        assert 'Already attached to default VRF' in ret, 'Adding default VRF validation failed'
        ...
        assert expected_output not in ret, 'Multiple Interface detach failed'
    ...
    def test_ip(self):
        ...
        # Deleting IP address on an L3 interface which does not have any IP address
        s1.cmdCLI("routing")
        ...
        intf_cmd = "/usr/bin/ovs-vsctl get port " + first_interface + " ip4_address"
        ...
        # Remove IP address on L3 interface by giving an IP address that is not present
        ret = s1.cmdCLI("no ip address 10.0.30.2/24")
        ...
        # Removing IP address from L3 interface giving correct IP address
        ...
        # Deleting secondary IP address on an L3 interface which does not have any secondary IP address
        ret = s1.cmdCLI("no ip address 10.0.30.2/24 secondary")
        expected_output = "Error: No secondary IP Address configured on interface " + first_interface
        assert expected_output in ret, 'Secondary IP address presence validation failed'
        ...
        intf_cmd = "/usr/bin/ovs-vsctl get port " + first_interface + " ip4_address_secondary"
        ip = s1.ovscmd(intf_cmd).strip()
        assert ip == '[10.0.20.4/24, 10.0.20.5/24]', 'Secondary IP address add failed'
        ...
        intf_cmd = "/usr/bin/ovs-vsctl get port " + first_interface + " ip4_address_secondary"
        ip = s1.ovscmd(intf_cmd).strip()
        assert ip == '[10.0.20.5/24]', 'Secondary IP address delete failed'
    ...
    def test_ipv6(self):
        ...
        intf_cmd = "interface " + first_interface
        s1.cmdCLI(intf_cmd)
        ret = s1.cmdCLI("ipv6 address 2002::1/120")
        expected_output = "Error: Interface " + first_interface + " is not L3."
        assert expected_output in ret, 'IPv6 address validation failed'
        info('### IPv6 address validation passed ###\n')

        # Deleting IPv6 address on an L3 interface which does not have any IPv6 address
        s1.cmdCLI("routing")
        ret = s1.cmdCLI("no ipv6 address 2002::1/120")
        expected_output = "Error: No IPv6 Address configured on interface " + first_interface
        assert expected_output in ret, 'IPv6 address presence validation failed'
        info('### IPv6 address presence validation passed ###\n')

        # Configuring IPv6 address on L3 interface
        s1.cmdCLI("ipv6 address 2002::1/120")
        intf_cmd = "/usr/bin/ovs-vsctl get port " + first_interface + " ip6_address"
        ...
        intf_cmd = "/usr/bin/ovs-vsctl get port " + first_interface + " ip6_address"
        ipv6 = s1.ovscmd(intf_cmd).strip()
        assert ipv6 == "2001::1/120", 'IPv6 address update failed'
        info('### IPv6 address updated successfully ###\n')

        # Remove IPv6 address on L3 interface by giving an IPv6 address that is not present
        ret = s1.cmdCLI("no ipv6 address 2004::1/120")
        assert "Error: IPv6 Address 2004::1/120 not found." in ret, 'IPv6 address delete validation failed'
        info('### IPv6 address delete validation passed ###\n')

        # Removing IPv6 address from L3 interface giving correct IPv6 address
        ret = s1.cmdCLI("no ipv6 address 2001::1/120")
        intf_cmd = "/usr/bin/ovs-vsctl get port " + first_interface + " ip6_address"
        ipv6 = s1.ovscmd(intf_cmd).strip()
        assert ipv6 == "[]", 'IPv6 address remove failed'
        info('### IPv6 address removed successfully ###\n')

        # Deleting secondary IPv6 address on an L3 interface which does not have any secondary IPv6 address
        ret = s1.cmdCLI("no ipv6 address 2004::1/120 secondary")
        expected_output = "Error: No secondary IPv6 Address configured on interface " + first_interface
        assert expected_output in ret, 'Secondary IPv6 address presence validation failed'
        info('### Secondary IPv6 address presence validation passed ###\n')

        # Configuring multiple secondary IPv6 addresses on L3 interface
        s1.cmdCLI("ipv6 address 2001::2/120 secondary")
        s1.cmdCLI("ipv6 address 2001::3/120 secondary")
        intf_cmd = "/usr/bin/ovs-vsctl get port " + first_interface + " ip6_address_secondary"
        ipv6 = s1.ovscmd(intf_cmd).strip()
        assert ipv6 == '[2001::2/120, 2001::3/120]', 'Secondary IPv6 address add failed'
        info('### Secondary IPv6 address added successfully ###\n')

        # Deleting multiple secondary IPv6 addresses on L3 interface
        s1.cmdCLI("no ipv6 address 2001::2/120 secondary")
        intf_cmd = "/usr/bin/ovs-vsctl get port " + first_interface + " ip6_address_secondary"
        ipv6 = s1.ovscmd(intf_cmd).strip()
        assert ipv6 == '[2001::3/120]', 'Secondary IPv6 address delete failed'
        info('### Secondary IPv6 address deleted successfully ###\n')
        s1.cmdCLI("no ipv6 address 2001::3/120 secondary")
        intf_cmd = "/usr/bin/ovs-vsctl get port " + first_interface + " ip6_address_secondary"
        ipv6 = s1.ovscmd(intf_cmd).strip()
    ...
    def test_toggle_l2_l3(self):
        ...
        '''
        info('\n########## Testing routing/ no routing working ##########\n')
        s1 = self.net.switches[ 0 ]
        ...
        ret = s1.cmdCLI("do show vrf")
        expected_output = '\t' + first_interface
        assert expected_output in ret, 'Interface is not L3. "routing" failed'
    ...
    def test_show_running_config(self):
        ...
        assert 'ip address 10.1.1.1/8' in output[intf2_output+1] and \
               'ip address 10.1.1.3/8 secondary' in output[intf2_output+2] and \
               'ipv6 address 2002::1/120' in output[intf2_output+3] and \
               'ipv6 address 2002::2/120 secondary' in output[intf2_output+4], \
                'Show running config for ' + intf2 + ' failed'
        info('### Show running config for ' + intf2 + ' passed ###\n')
        assert 'no lldp reception' in output[intf3_output+1] and \
               'no routing' in output[intf3_output+2], 'Show running ' \
