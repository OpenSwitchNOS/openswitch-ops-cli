...
class pingCLITest(OpsVsiTest):
    ...
    def pingValidationIpv4Test(self):
        ...
        assert 'pattern for datafill should be in hexadecimal only' in ret, \
        ...
        info('\n### Ping IP: ip-option include-timestamp validation passed ###\n')

        ret = s1.cmdCLI("ping 1.1.1.1 ip-option include-timestamp-and-address 3")
        assert '% Unknown command.' in ret, \
            'Ping IP :ip-option include-timestamp-address validation failed'
        info('\n### Ping IP: ip-option include-timestamp-address validation passed ###\n')
    ...
    def pingValidationIpv4HostTest(self):
        ...
        info('\n### Ping Host: ip-option include-timestamp validation passed ###\n')

        ret = s1.cmdCLI("ping testname ip-option include-timestamp-and-address 3")
        assert '% Unknown command.' in ret, \
            'Ping Host :ip-option include-timestamp-address validation failed'
        info('\n### Ping Host: ip-option include-timestamp-address validation passed ###\n')
