...
static int set_global_reinit_time(const char *reinit_time)
{
  const struct ovsrec_system *row = NULL;
  enum ovsdb_idl_txn_status status;
  struct ovsdb_idl_txn* status_txn = cli_do_config_start();
  ...
  if( SYSTEM_OTHER_CONFIG_MAP_LLDP_REINIT_DEFAULT != atoi(reinit_time))
    smap_replace(&smap_other_config, SYSTEM_OTHER_CONFIG_MAP_LLDP_REINIT, reinit_time);
  else
...
DEFUN (cli_lldp_set_reinit_time,
       ...
       "lldp reinit <1-10>",
       CONFIG_LLDP_STR
       "Set reinit timer, time to wait before performing LLDP initialization on any interface.\n"
       ...
       "Reinit time (Default:2)\n")
{
  ...
  snprintf(def_reinit_time, LLDP_TIMER_MAX_STRING_LENGTH, "%d", SYSTEM_OTHER_CONFIG_MAP_LLDP_REINIT_DEFAULT );
