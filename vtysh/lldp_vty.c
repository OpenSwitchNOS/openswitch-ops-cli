...
static int lldp_set_global_status(const char *status)
{
  ...
  smap_clone(&new_map, &row->other_config);

  if(strcmp("true",status) == 0)
    smap_replace(&new_map, OPEN_VSWITCH_OTHER_CONFIG_MAP_LLDP_ENABLE, status);
  else
    smap_remove(&new_map, OPEN_VSWITCH_OTHER_CONFIG_MAP_LLDP_ENABLE);

  ovsrec_open_vswitch_set_other_config(row, &new_map);

  smap_destroy(&new_map);
...
static int set_global_hold_time(const char *hold_time)
{
  ...
  if( OPEN_VSWITCH_OTHER_CONFIG_MAP_LLDP_HOLD_DEFAULT != atoi(hold_time))
    smap_replace(&new_map, OPEN_VSWITCH_OTHER_CONFIG_MAP_LLDP_HOLD, hold_time);
  else
    smap_remove(&new_map, OPEN_VSWITCH_OTHER_CONFIG_MAP_LLDP_HOLD);
...
static int lldp_set_global_timer(const char *timer)
{
  ...
  if(OPEN_VSWITCH_OTHER_CONFIG_MAP_LLDP_TX_INTERVAL_DEFAULT != atoi(timer))
    smap_replace(&new_map, OPEN_VSWITCH_OTHER_CONFIG_MAP_LLDP_TX_INTERVAL, timer);
  else
    smap_remove(&new_map, OPEN_VSWITCH_OTHER_CONFIG_MAP_LLDP_TX_INTERVAL);
       ...
       "Clear LLDP counters.\n")
{
  ...
  smap_clone(&new_map, &row->status);

  clear_counter = smap_get_int(&row->status, "lldp_num_clear_counters_requested", 0);
  clear_counter++;

  sprintf(buffer, "%d", clear_counter);
  smap_replace(&new_map, "lldp_num_clear_counters_requested", buffer);
  ovsrec_open_vswitch_set_status(row, &new_map);
       ...
       "Clear LLDP neighbor tables.\n")
{
  ...
  smap_replace(&new_map, "lldp_num_clear_table_requested", buffer);
  ovsrec_open_vswitch_set_status(row, &new_map);
...
lldp_set_tlv(const char *tlv_name, const char *status)
{
  ...
  if(strcmp("false",status) == 0)
    smap_replace(&new_map, tlv, status);
  else
    smap_remove(&new_map, tlv);
...
static int lldp_set_mgmt_address(const char *status, boolean set)
{
  ...
  if(set)
    smap_replace(&new_map, OPEN_VSWITCH_OTHER_CONFIG_MAP_LLDP_MGMT_ADDR, status);
  else
    smap_remove(&new_map, OPEN_VSWITCH_OTHER_CONFIG_MAP_LLDP_MGMT_ADDR);
...
static inline int compar_intf(const void*a,const void* b)
{
  ...
  OVSREC_INTERFACE_FOR_EACH(row, idl)
  {
    if(strcmp(row->name, ifvalue) == 0)
    {
      ...
      if(state == LLDP_TX)
      {
        ...
        {
          smap_replace(&new_map, INTERFACE_OTHER_CONFIG_MAP_LLDP_ENABLE_DIR,
      ...
      ovsrec_interface_set_other_config(row, &new_map);
...
int lldp_ovsdb_if_lldp_nodirstate(const char *ifvalue, const lldp_tx_rx state)
{
  ...
  OVSREC_INTERFACE_FOR_EACH(row, idl)
  {
    if(strcmp(row->name, ifvalue) == 0)
    {
      ...
      ifexists = true;
      state_value = smap_get(&new_map, INTERFACE_OTHER_CONFIG_MAP_LLDP_ENABLE_DIR);
      if(state == LLDP_TX)
      {
