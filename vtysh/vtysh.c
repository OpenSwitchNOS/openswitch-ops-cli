...
#endif
static rbac_role_t* grbac_role = NULL;
...
bool rbac_get_user_role_plugin( char * username)
{
 lt_dlhandle dhhandle = 0;
 rbac_role_t *temp_role = NULL;
 bool (*fun_ptr)(const char *, rbac_role_t *) = NULL;
 lt_dlinit();
 lt_dlerror();
 ...
 fun_ptr = lt_dlsym(dhhandle,"rbac_get_user_role");
 ...
 if(lt_dlerror() || fun_ptr == NULL)
 {
  ...
  temp_role = calloc(1, sizeof(rbac_role_t));
  ...
  if((*fun_ptr)(username,temp_role))
  {
     grbac_role = temp_role;
     return true;
  }
  else{
     free(temp_role);
...
bool check_user_role(char *username, char *role)
{
  ...
  if(grbac_role == NULL)
  {
     if(!rbac_get_user_role_plugin(username))
     {
        return false;
     }
  }

  if(strcmp( grbac_role->name, role) == 0)
  {
...
#endif /* ENABLE_OVSDB */
  pw = getpwuid( getuid());
  if ( pw && !(check_user_role(pw->pw_name,RBAC_ROLE_NETOP)))
  {
