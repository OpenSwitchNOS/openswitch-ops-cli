...
copy_ovsdb_tempdb()
{
    ...
    {
        vty_out(vty, "Internal error occured, Try again!%s", VTY_NEWLINE);
...
int remove_temp_db(int initialize)
{
  ...
  if (fp == NULL)
  {
      ...
      {
          vty_out(vty, "Internal error occured, Try again!%s", VTY_NEWLINE);
          VLOG_ERR("Error while opening the file\n");
          fclose(fp);
          return -1;
      }
      else
  ...
  if(file_not_present == 1 && execute_command ("rm", 1, (const char **)remove_db) == -1)
  {
      if (initialize == 0)
      {
       ...
       "Contents of startup configuration\n")
{
  ...
  // Check if temporary DB existsi and server running, if yes remove it.
  remove_temp_db(1);
