...
/* Path to the public key generated by the password server on daemon start */
#define PASSWDSRV_PUBKEY        "/var/run/ops-passwd-srv/ops-passwd-srv-pub.pem"
#define PASSWDSRV_SOCKET        "/var/run/ops-passwd-srv/ops-passwd-srv.sock"
...
static unsigned char *encrypt_msg_to_passwd_srv_d(passwd_srv_msg_t in)
{
    ...
    pubkey = RSA_new();
    pubkeyfile = fopen(PASSWDSRV_PUBKEY, "r");
     ...
     * this length with padding type RSA_PKCS1_PADDING (see man
     * RSA_public_encrypt) for details */
    maxEncryptLen = RSA_size(pubkey) - 12;
    ...
    ret = RSA_public_encrypt(sizeof(passwd_srv_msg_t), (unsigned char *) &in,
                             enc_msg, pubkey, RSA_PKCS1_PADDING);
        ...
        const char* newpass)
{
    ...
    strncpy(unSrv.sun_path, PASSWDSRV_SOCKET, strlen(PASSWDSRV_SOCKET));
    ...
    /* our rsa key is 2048 bit or 256 byte, the encrypted message is necessarily
     * the same length, in the future this will be inherited from a header file,
     * for now I have hard coded it so that our struct RSA pubkey can be erased
     * and freed when it is no longer needed */
    if (0 > send(sockfd, enc_msg, 256, MSG_DONTROUTE))
    {
