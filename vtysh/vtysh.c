...
static unsigned char *encrypt_msg_to_passwd_srv_d(passwd_srv_msg_t in)
{
    ...
    if (pubkeyfile == NULL)
    {
        /* TODO: log: could not open public key file */
        return NULL;
    }
    ...
    if (pubkey == NULL) {
        /* TODO: log: could not import public key */
        goto cleanup;
        success = 0;
    }
    ...
    enc_msg = (unsigned char*) malloc(RSA_size(pubkey));
...
{
    int sockfd, sockaddr_len, msg_len;
    ...
    if (NULL == username)
    {
        ...
        vty_out(vty, "Invalid argument - username required", VTY_NEWLINE);
        return;
    }
    ...
    case PASSWD_MSG_ADD_USER:
    {
        if (NULL == newpass)
        {
            vty_out(vty, "Invalid argument - require new password", VTY_NEWLINE);
            return;
        }
        /* we are adding user */
        msg.op_code = opcode;
    ...
    case PASSWD_MSG_CHG_PASSWORD:
    {
        ...
        {
            vty_out(vty, "Invalid argument - require new & old password", VTY_NEWLINE);
            return;
        }
    ...
    default:
    {
        vty_out(vty, "Invalid argument", VTY_NEWLINE);
        return;
    }
    }
    ...
    enc_msg = encrypt_msg_to_passwd_srv_d(msg);
    if (enc_msg == NULL)
    {
    ...
    {
        vty_out(vty, "%s failed [conn failed].%s", op, VTY_NEWLINE);
        return;
    }
    ...
    {
        vty_out(vty, "%s failed [sent failed].%s", op, VTY_NEWLINE);
    ...
    {
        vty_out(vty, "%s failed [recv failed].%s", op, VTY_NEWLINE);
    ...
    {
        vty_out(vty, "Failed to decrypt message.%s", VTY_NEWLINE);
    }
    default:
        vty_out(vty, "Unknown error recv'd by server [error=%d].%s", error_code,
                VTY_NEWLINE);
        break;
    }
