 ...
 * Source for registering source interface client callback with openvswitch table.
 *
 ***************************************************************************/
...
char source_interface_context_client_name[] = "vtysh_source_interface_context_clientcallback";
...
vtysh_source_interface_context_clientcallback (void *p_private)
{
    ...
    source_interface_buff = (char *)smap_get(&row->other_config, "source_interface_all");
    ...
    if (source_interface_buff !=NULL) {
        ...
        vtysh_ovsdb_cli_print(p_msg, "%4s%s", "", "N/A");
    }

    source_interface_buff = (char *)smap_get(&row->other_config, "source_interface_tftp");
    vtysh_ovsdb_cli_print(p_msg, "%s", "source interface");
    if (source_interface_buff !=NULL) {
        if (!source_flag) {
             vtysh_ovsdb_cli_print(p_msg, "%4s%s", "", "source_interface_buff");
