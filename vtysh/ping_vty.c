    ...
    switch(type)
    {
        case IPv4_DESTINATION :
        {
            ...
            if (isalpha((int) *value))
            {
                ...
                {
                    vty_out (vty, "Invalid Hostname : Hostname length should be less than 256 characters %s", VTY_NEWLINE);
                    return CMD_WARNING;
                }
                p->isIpv4 = true;
                length = strlen((char*)value);
                p->pingTarget = (char*)malloc(length+1);
        ...
        case DATA_FILL :
        {
            ...
            if(value)
            {
                ...
                for(i=0;store[i]!='\0';i++)
                {
                    ...
                    {
                        vty_out (vty, "pattern for datafill should be in hexadecimal only %s", VTY_NEWLINE);
                        return CMD_WARNING;
                    }
                    else
