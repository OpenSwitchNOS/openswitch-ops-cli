...
def configure_route_map_set_commands(dut, routemap):
    if (enterConfigShell(dut) is False):
        return False
    cmd1 = "route-map "+routemap+" permit 20"
    cmd2 = "set community 2:0 3:0 4:0"
    ...
    devIntReturn = dut.DeviceInteract(command=cmd1)
    devIntReturn = dut.DeviceInteract(command=cmd2)
...
def verify_routemap_set(**kwargs):
    LogOutput('info', "\n\n########## Verifying route-map set commands \
    ##########\n")
    ...
    assert set_community_flag, "Failed to configure 'set community'"
    LogOutput('info', "'set community' running succesfully")
    assert set_commlist_flag, "Failed to configure 'set comm-list'"
    LogOutput('info', "'set comm-list' running succesfully")
    assert set_weight_flag, "Failed to configure 'set weight'"
    LogOutput('info', "'set weight' running succesfully")
    assert set_aggregator_flag, "Failed to configure 'set aggregator as'"
    LogOutput('info', "'set aggregator as' running succesfully")
    assert set_atomic_flag, "Failed to configure 'set atomic'"
    LogOutput('info', "'set atomic' running succesfully")
    assert set_localpref_flag, "Failed to configure 'set local-preference'"
...
class Test_bgp_routemap:
    def setup_class(cls):
        ...
        #    Get topology object
        Test_bgp_routemap.topoObj = \
            Test_bgp_routemap.testObj.topoObjGet()

    def teardown_class(cls):
        Test_bgp_routemap.topoObj.terminate_nodes()
