...
# Verify to configure system domainname  through CLI without input
    ...
    returnStructure = device1.DeviceInteract(command="domain-name")
...
# Verify to configure system domainname through CLI
    ...
    returnStructure = device1.DeviceInteract(command="domain-name cli")
    returnStructure = device1.DeviceInteract(command="ovs-vsctl list \
                                             domain_name")
    bufferout = returnStructure.get('buffer')
    assert 'domain_name cli' not in bufferout, "Test to configure domainname \
...
# Verify to display system domainname through CLI
    LogOutput('info', "\n\n######## Test to display configured system "
              "domainname through CLI ########")
    returnStructure = device1.DeviceInteract(command="show domain-name")
    bufferout = returnStructure.get('buffer')
...
# Verify to reset configured system domainname through CLI
    ...
    returnStructure = device1.DeviceInteract(command="no domain-name")
    returnStructure = device1.DeviceInteract(command="ovs-vsctl list \
                                                      domain_name")
    bufferout = returnStructure.get('buffer')
    assert 'domain_name ' not in bufferout, "Test to reset configured \
            domainname through CLI has failed"
